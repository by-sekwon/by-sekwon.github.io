...
동일하다. '범인은 무죄이다'를 시작으로
'검사가 제시한 여러 증거'(표본 데이터)를
...
모수 $(\theta \in \Omega = \left\{ \theta;\omega_{0} \cup \omega_{1} \right\},\omega_{0} \cap \omega_{1} = \phi)$에
...
지지되느냐는 관측된 표본 데이터에 의존한다. "귀무"(null)의
의미는 아무 것도 없다는 것이다.
:::
...
::: {.callout-note title="1종 오류, 2종 오류"}
...
$f(x;\theta) = \frac{1}{\theta},0 < x < \theta \sim U(0,\theta)$에서 모수 $\theta$에 대한 다음 가설을 검정하기 위하여 확률표본을 추출하였다.
...
::: {.callout-tip title="예제"}
A 교수는 수리통계학 시험 점수가 평균 70점 수준으로 출제했다고
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim N(\mu,\sigma^{2})$에서 표본 크기 $n$인
...
$T(\overline{x}) = \frac{\sqrt{n}(\bar{x} - \mu_{0})}{S} \sim t(n - 1)$이다.(자세한 내용은 다음 2절. 우도비 검정을 참고하기 바란다.)
...
::: {.callout-note title="유의확률과 대립가설 형태"}
...
1. 양측검정이냐? 단측검정이냐? 우리가 무엇에 관심을 갖느냐 하는데 있다. 불량률과 같이 \~값 미만에 관심이 있다면 왼쪽이 단측 검정, 수익과 같이 \~값 이상에 관심이 있다면 우측이 대립가설이 단측 검정을 실시한다. "차이가 없다" 같이 양쪽 모두에 관심이 있다면 양측검정을 실시한다.
...
$\overset{\hat{}}{\theta} = \bar{X}$, 표본분산 $S^{2}$, 귀무가설
...
::: {.callout-tip title="예제"}
정부는 PC 구입 가격이 900\$이라고 발표했다. 이를 알아보기
위하여 소비자 55명을 임의 조사하였더니 평균 885\$, 표준편차는
50\$이었다. 

- 모집단 평균에 대한 99%(양측)신뢰구간을 구하시오. ($\bar{X} \pm Z_{0.995}\frac{S}{\sqrt{n}}) \rightarrow (885 \pm 2.58\frac{50}{\sqrt{25}}) \rightarrow (859.2,910.8)$

- 정부의 발표가 진실이라고 할 수 있나(유의수준 1%)? 유의수준 1% 검정 통계량 :
$\frac{\bar{X} - \mu_{0}}{\frac{S}{\sqrt{n}}} = \frac{885 - 900}{\frac{50}{\sqrt{25}}} = - 0.5$
...
::: {.callout-tip title="예제"}
$f(x;\theta = \lambda) \sim Poisson(\theta)$의 모수 $\theta$에 대한 가설 검정을 위하여 표본크기 $n = 10$인 확률표본
...
::: {.callout-note title="유의수준"}
확률표본의 영역을 $\mathcal{S}$, 확률표본 영역의 부분 집합
...
::: {.callout-note title="검정력"}
다음을 검정력 power function 이라 정의한다.
...
::: {.callout-important title="최강 검정력"}
확률표본의 영역을 $\mathcal{S}$, 확률표본 영역의 부분
...
::: {.callout-tip title="예제"}
모집단 $X \sim B(p = \theta)$ 에서 귀무가설 $H_{0}:p = 0.1$,
...
::: {.callout-important title="most powerful test 최강력 검정법"}
...
::: {.callout-important title="Neyman-Pearson Lemma"}
단순 귀무가설 $H_{0}:\theta = \theta_{0}$와
...
$\frac{L({\overline{x};\theta}_{0})}{L({\overline{x};\theta}_{1})} \leq k, \text{ for } \overline{x} \in RR$.

$\frac{L({\overline{x};\theta}_{0})}{L({\overline{x};\theta}_{1})} \geq k, \text{ for } \overline{x} \in RR^{C}$.

$${{\alpha = P}_{H}}_{0}(\overline{x} \in RR)$$
:::

::: {.callout-note title="불편 검정방법"}
모집단 $X \sim f(x;\theta),\theta \in \Omega$ 에서
확률표본 $\overline{X} = \left( X_{1},X_{2},\ldots,X_{n} \right)$,
기각역은 ${{\alpha = P}_{H}}_{0}(\overline{x} \in RR)$이라 하면 다음을
불편 unbiased 검정방법이다. $P_{\theta}\left( \overline{X} \in RR \right) \geq \alpha, \text{ for all } \theta \in \omega_{1}$
:::

::: {.callout-tip title="예제"}
확률표본 $\overline{X} = \left( X_{1},X_{2},\ldots,X_{n} \right) \sim f(x;\theta = \mu)\sim N(\mu,1)$이고
...
${P_{H}}_{0}\left( \overline{x} \geq c \middle| \mu = 0 \right) = \int_{c}^{\infty}{\frac{1}{\sqrt{2\pi}\sqrt{1/n}}\exp\left( - \frac{\left( \bar{x} - 0 \right)^{2}}{2(1/n)} \right)d\bar{x}} = \alpha$을
만족하는 $c$ 값을 기각역의 시작 값인 임계값 $c_{1}$이다.

검정력:
${P_{H}}_{1}\left( \overline{x} \geq c_{1} \middle| \mu = 1 \right) = \int_{c_{1}}^{\infty}{\frac{1}{\sqrt{2\pi}\sqrt{1/n}}\exp\left( - \frac{\left( \bar{x} - 1 \right)^{2}}{2(1/n)} \right)d\bar{x}}$
:::

::: {.callout-tip title="예제"}
확률표본 $\overline{X} = \left( X_{1},X_{2},\ldots,X_{n} \right) \sim f(x)$이고 귀무가설, 대립가설이 아래와 같이 확률분포함수인 경우 최강 기각역을 구하라.
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim B(p),x = 0,1$에서 모수 $\theta = p$에
...
::: {.callout-important title="일양 최강 검정법"}
귀무가설 $H_{0}:\theta = \theta_{0}$, 단순 대립가설
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) = \frac{1}{\theta}e^{- x/\theta},x > 0$에서
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim N(0,\theta)$에서 모수 $\theta$(분산)에
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim B(\theta = p),0 < \theta < 1$에서 모수
...
::: {.callout-important title="지수족 UMPT"}
...
::: {.callout-note title="우도비 검정"}
귀무가설 $H_{0}:\theta \in \Omega_{0}$, 대립가설
...
$\overset{\hat{}}{\mu} = max(\bar{x},\mu_{0})$이다. 왜냐하면 모수
...
\left( \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{\sum\left( x_{i} - \mu_{0} \right)^{2}} \right)^{\frac{n}{2}} \\
1, \text{ if } \overline{x} \leq \mu_{0}
\end{array}, \text{ if } \overline{x} > \mu_{0} \right.\ $ .

$\sum\left( x_{i} - \mu_{0} \right)^{2} = \sum\left( x_{i} - \bar{x} \right)^{2} + \sum\left( \bar{x} - \mu_{0} \right)^{2}$
이므로 정리하면

$$\lambda = \left( \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{\sum\left( x_{i} - \mu_{0} \right)^{2}} \right)^{\frac{n}{2}} \leq k \Leftrightarrow \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{\sum\left( x_{i} - \mu_{0} \right)^{2}} \leq k^{\frac{2}{n}} \Leftrightarrow \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{\sum\left( x_{i} - \bar{x} \right)^{2} + \sum\left( \bar{x} - \mu_{0} \right)^{2}} \leq k^{\frac{2}{n}}$$

$$\Rightarrow \frac{\sqrt{n}(\bar{x} - \mu_{0})}{S} > c, \text{ where } S^{2} = \sum\left( x_{i} - \bar{x} \right)^{2}/(n - 1)$$

$\frac{\sqrt{n}(\bar{x} - \mu_{0})}{S}$은 t-분포를 따르므로
우도비 검정은 t-검정과 일치한다.
:::

::: {.callout-tip title="예제"}
모집단 $f(x;\theta) = e^{- (x - \theta)},x \geq 0$에서 표본
크기 $n$인 확률표본
$\overline{X} = \left( X_{1},X_{2},\ldots,X_{n} \right)$을 추출하였다.
...
e^{- n\left( x_{(1)} - \theta_{0} \right)}, \text{ if } x_{(1)} > \theta_{0} \\
1, \text{ if } x_{(1)} \leq \theta_{0}
\end{array} \right.\ $
...
$\lambda = \frac{L(\overset{\hat{}}{\Omega_{0}})}{L(\overset{\hat{}}{\Omega})}$라 정의하자. 표본의 크기 $n$이 충분히 크다면 귀무가설
...
::: {.callout-tip title="예제"}
작업 라인이 2개 있다. 각 작업을 일주일 단위로 생산된 제품에
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim N(\mu,1)$에서 모수 $\theta$ 추론을
위하여 표본 크기 2인 확률표본
$\overline{X} = \left( X_{1},X_{2} \right)$ 추출하였다. 모수 $\theta$에
대한 신뢰구간을 구하라.

모수 $\theta$의 MVUE가 $\bar{X}$이므로
$(\bar{X} - 1,\bar{X} + 2)$ 신뢰구간 중 하나이다. 신뢰수준
: 91.9%
$P\left( \bar{X} - 1 \leq \mu \leq \bar{X} + 2 \middle| \bar{X} \sim N\left( \mu,\frac{1}{2} \right) \right) = P\left( - \frac{2}{\sqrt{1/2}} \leq z = \frac{\bar{X} - \mu}{\sqrt{1/2}} \leq \frac{1}{\sqrt{1/2}} \right) = P\left( - 2\sqrt{2} \leq z \leq \sqrt{2} \right) = 0.921 - 0.002 = 0.919$
:::
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim U(0,\theta)$에서 표본 크기 $n$인
확률표본 $\overline{X} = \left( X_{1},X_{2},\ldots,X_{n} \right)$
...
::: {.callout-note title="유의수준 $\alpha$ 가설검정과 $100(1 - \alpha)\%$ 신뢰구간은 동일하다."}
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim exponetial(\theta = \lambda)$에서 표본
크기 $n$인 확률표본
$\overline{X} = \left( X_{1},X_{2},\ldots,X_{n} \right)$ 추출하였다.
...
(모수 $\lambda$의 MLE는 $\bar{x}$ 이기 때문이다)
...
::: {.callout-tip title="예제"}
$f(x;\theta) \sim N\left( \theta = \mu,\sigma^{2} \right),\text{ where } \sigma^{2} \text{ is known}$에서
...
::: {.callout-important title="cochran theorem"}
...
::: {.callout-tip title="예제"}
모집단 $f(x;\theta) \sim N\left( \mu,\sigma^{2} \right)$에서
확률표본 $\left( X_{1},X_{2},\ldots,X_{n} \right)$의 분산 $\sigma^{2}$의
MVUE 표본 분산
$S^{2} = \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{n - 1}$ 은
이차형식이다.
$(n - 1)S^{2} = \sum\left( x_{i} - \bar{x} \right)^{2} \Leftrightarrow \frac{(n - 1)S^{2}}{\sigma^{2}} = \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{\sigma^{2}} = \frac{\sum\left( x_{i} - \mu \right)^{2}}{\sigma^{2}} - \frac{\sum\left( \bar{x} - \mu \right)^{2}}{\sigma^{2}}$
...
이므로 $Q_{1} = \frac{{n\left( \bar{X} - \mu \right)}^{2}}{\sigma^{2}} \sim \chi^{2}(1$)
이다. 그러므로 코크란 정리에 의해
...
::: {.callout-note title="표본분산 분할"}

분산분석 모형에서 표본분산
$S^{2} = \frac{1}{(an - 1)}\sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{..} \right)^{2}}$을
이차형식으로 분할하라. 단,
${\bar{x}}_{..} = \frac{\sum_{i}^{}{\sum_{j}^{}x_{ij}}}{an},{\bar{x}}_{i.} = \frac{\sum_{j}^{}x_{ij}}{n}$,
${\bar{x}}_{.j} = \frac{\sum_{i}^{}x_{ij}}{a}$이다.

${(an - 1)S}^{2} = \sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{..} \right)^{2}} = \sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{i.} \right)^{2}} + \sum_{i}^{}{\sum_{j}^{}\left( {\bar{x}}_{i.} - {\bar{x}}_{..} \right)^{2}}$

$= \sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{i.} \right)^{2}} + n\sum_{i}^{}{\left( {\bar{x}}_{i.} - {\bar{x}}_{..} \right)^{2} = Q_{1} + Q_{2}}$

${(an - 1)S}^{2} = \sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{..} \right)^{2}} = \sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{.j} \right)^{2}} + \sum_{i}^{}{\sum_{j}^{}\left( {\bar{x}}_{.j} - {\bar{x}}_{..} \right)^{2}}$

$= \sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{.j} \right)^{2}} + a\sum_{j}^{}{\left( {\bar{x}}_{.j} - {\bar{x}}_{..} \right)^{2} = Q_{3} + Q_{4}}$

$${(an - 1)S}^{2} = a\sum_{j}^{}{\left( {\bar{x}}_{.j} - {\bar{x}}_{..} \right)^{2} +}n\sum_{i}^{}{\left( {\bar{x}}_{i.} - {\bar{x}}_{..} \right)^{2} +}\sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{i.} - {\bar{x}}_{.j} + {\bar{x}}_{..} \right)^{2}} = Q_{2} + Q_{3} + Q_{4}$$
:::
...
- $\mu$  MLE :
$\frac{\partial L\left( \Omega_{0} \right)}{\partial\mu} = 0 \Rightarrow \overset{\hat{}}{\mu} = \frac{1}{an}\sum_{i}^{}{\sum_{j}^{}x_{ij} = {\bar{x}}_{..}}$

- $\sigma^{2}$  MLE :
$\frac{\partial L\left( \Omega_{0} \right)}{\partial\sigma^{2}} = 0 \Rightarrow \overset{\hat{}}{\sigma^{2}} = \frac{1}{an}\sum_{i}^{}{\sum_{j}^{}{{(x}_{ij} - {\bar{x}}_{..})^{2}} = v}$

- 분모 우도함수 :
$L(\Omega) = \left( \frac{1}{2\pi\sigma} \right)^{an/2}exp( - \frac{1}{2\sigma^{2}}\sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - \mu_{i} \right)^{2}})$

- $\mu_{i}$  MLE :
$\frac{\partial L(\Omega)}{\partial\mu_{i}} = 0 \Rightarrow \overset{\hat{}}{\mu_{i}} = \frac{1}{n}\sum_{j}^{}{x_{ij} = {\bar{x}}_{i.}}$

- $\sigma^{2}$  MLE :
$\frac{\partial L(\Omega)}{\partial\sigma^{2}} = 0 \Rightarrow \overset{\hat{}}{\sigma^{2}} = \frac{1}{an}\sum_{i}^{}{\sum_{j}^{}{{(x}_{ij} - {\bar{x}}_{..})^{2}} = w}$

- 우도비 :
$\lambda = \frac{L\left( {\overset{\hat{}}{\Omega}}_{0} \right)}{L\left( \overset{\hat{}}{\Omega} \right)} = \left( \frac{\sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{i.} \right)^{2}}}{\sum_{i}^{}{\sum_{j}^{}\left( x_{ij} - {\bar{x}}_{..} \right)^{2}}} \right)^{an/2}$.

- 가정: $e_{ij} \sim (iid)N(0,\sigma^{2})$에 의해
$X_{ij} \sim (iid)N(\mu_{i},\sigma^{2})$ 이다.

그러므로 $V = \frac{1}{an}\sum_{i}^{}{\sum_{j}^{}{{(x}_{ij} - {\bar{x}}_{..})^{2}} = \frac{Q}{an}}$ 이고 $W = \frac{1}{an}\sum_{i}^{}{\sum_{j}^{}{{(x}_{ij} - {\bar{x}}_{i.})^{2}} = \frac{Q_{3}}{an}}$
이다.
...
::: {.callout-note title="ANOVA table"}
...
::: {.callout-note title="OLS 최소자승추정법"}
...
$\overset{\hat{}}{b} = \frac{\sum\left( x_{i} - \bar{x} \right)\left( y_{i} - \bar{y} \right)}{\sum\left( x_{i} - \bar{x} \right)^{2}} = \frac{S_{XY}}{S_{XX}}$,
$\overset{\hat{}}{a} = \bar{y} - \overset{\hat{}}{b}\bar{x}$
:::

::: {.callout-note title="MLE"}
...
::: {.callout-important title="OLS is blue"}
markov theorem
...
::: {.callout-note title="오차 분산 MVUE 추정량"}

$\overset{\hat{}}{\sigma^{2}} = \frac{1}{n - 2}\sum\left( y_{i} - \overset{\hat{}}{a} - \overset{\hat{}}{b}x_{i} \right)^{2} = MSE$.
:::

::: {.callout-note title="$\overset{\hat{}}{b}$ 샘플링 분포"}

$\overset{\hat{}}{b} = \frac{\sum\left( x_{i} - \bar{x} \right)\left( y_{i} - \bar{y} \right)}{\sum\left( x_{i} - \bar{x} \right)^{2}}$
이므로
$k_{i} = \frac{\left( y_{i} - \bar{y} \right)}{\sum\left( x_{i} - \bar{x} \right)^{2}}$
이라 하면 $\overset{\hat{}}{b} = \sum k_{i}y_{i}$ 이다. 그리고
$\sum k_{i} = 0,\sum k_{i}x_{i} = 1,\sum k_{i}^{2} = \frac{1}{\sum\left( x_{i} - \bar{x} \right)^{2}}$이다.
...
$$V\left( \overset{\hat{}}{b} \right) = V\left( \sum k_{i}y_{i} \right) = \sum k_{i}^{2}{V(y}_{i}) = \sum k_{i}^{2}(\sigma^{2}) = \frac{\sigma^{2}}{\sum\left( x_{i} - \bar{x} \right)^{2}}$$

$\overset{\hat{}}{b} \sim N(b,\frac{\sigma^{2}}{\sum\left( x_{i} - \bar{x} \right)^{2}})$.
...
$s^{2}\left( \overset{\hat{}}{b} \right) = \frac{MSE}{\sum\left( x_{i} - \bar{x} \right)^{2}}$이다.
:::

::: {.callout-note title="기울기 $b$ 에 대한 $100(1 - \alpha)\%$ 신뢰구간"}
...
$s^{2}\left( \overset{\hat{}}{b} \right) = \frac{MSE}{\sum\left( x_{i} - \bar{x} \right)^{2}}$.
:::

::: {.callout-note title="절편 $a$ 에 대한 $100(1 - \alpha)\%$ 신뢰구간"}
...
$s^{2}\left( \overset{\hat{}}{a} \right) = MSE(\frac{1}{n} + \frac{{\bar{x}}^{2}}{\sum\left( x_{i} - \bar{x} \right)^{2}}$)
:::
