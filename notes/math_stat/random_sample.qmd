...
![](images/탐색적 확증적.png){fig-align="center" width="80%"}
...
![](images/확률표본.png){fig-align="center" width="40%"}
...
일반적으로 $f(x)$에 대한 관심보다는 요약값인 $\theta$에 관심을
...
$$f(x_{1},\ldots,x_{n} \mid \theta) = \prod_{i=1}^{n}f(x_{i} \mid \theta)$$
...
예를 들어 이항분포의 모수는 실험 회수($n$)와 성공확률($p$)이고,
...
확률표본으로부터 모수에 "가장" 적절한 값, 통계량(확률표본의
...
![](images/모집단 통계량.png){fig-align="center" width="60%"}
...
확률변수 $X_{1},\ldots,X_{n}$이 평균이 $\beta$인
지수분포로부터 추출된 크기 n의 확률표본이라고 하자. 결합확률밀도함수는
다음과 같다.

$$f(x_{1},\ldots,x_{n} \mid \beta) = \prod_{i=1}^{n}f(x_{i} \mid \beta) = \prod_{i=1}^{n}\frac{1}{\beta}e^{- x_{i}/\beta} = \frac{1}{\beta^{n}}e^{- (x_{1} + \cdots + x_{n})/\beta}$$

만약 모든 $X_{i}$가 1 이상 값을 가질 확률은

$$P(X_{1} > 1,\ldots,X_{n} > 1) = \int_{1}^{\infty}\cdots\int_{1}^{\infty}\prod_{i=1}^{n}\frac{1}{\beta}e^{- x_{i}/\beta}dx_{1}\cdots dx_{n}$$
...
$X_{2}$의 주변확률분포:
$P(X_{2} = x) = \sum_{i=1}^{N}P(X_{2} = x \mid X_{1} = x_{i}) \cdot P(X_{1} = x_{i})$
...
::: {.callout-tip title="예제 ②"}
유한 모집단 $\{ 1,2,\ldots,1000\}$에서 표본을 추출한다고
...
통계량의 정의는 매우 폭넓지만, 단 하나의 제한 조건은 통계량은 모수의
...
$(X_1, \ldots, X_n)$ 이 어떤 확률실험에서 얻어진 확률벡터(random vector)라고 하자.
우리는 종종 $T = T(X_1, \ldots, X_n)=\sum_{i=1}^{n} a_i X_i,$ 과 같은 선형결합(linear combinations)에 관심을 갖는다.
:::

::: {.callout-important title="정리"}
$T = \sum_{i=1}^{n} a_i X_i$ 라고 하자.
또한 모든 $i = 1, \ldots, n에 대해 E[|X_i|] < \infty$ 라고 하자. 그러면, $E(T) = \sum_{i=1}^{n} a_i E(X_i)$.
:::

::: {.callout-important title="정리"}
$T = \sum_{i=1}^{n} a_i X_i$ 이고, $W = \sum_{j=1}^{m} b_j Y_j$ 라고 하자. 만약 모든 $i = 1, \ldots, n$ 에 대해 $E[X_i^2] < \infty$, 그리고 모든 $j = 1, \ldots, m$ 에 대해 $E[Y_j^2] < \infty$ 라면,

$\operatorname{Cov}(T, W)
= \sum_{i=1}^{n} \sum_{j=1}^{m} a_i b_j \operatorname{Cov}(X_i, Y_j)$.
:::

::: {.callout-important title="따름정리"}
$T = \sum_{i=1}^{n} a_i X_i$ 라고 하자.
또한 모든 $i = 1, \ldots, n에 대해 E[X_i^2] < \infty$ 라고 하자. 그러면,
$\operatorname{Var}(T)
...
::: {.callout-important title="따름정리"}
만약 $X_1, \ldots, X_n$ 이 서로 독립이며 유한한 분산을 가진 확률변수들이라면, $\operatorname{Var}(T)
= \sum_{i=1}^{n} a_i^{2}\, \operatorname{Var}(X_i)$.
:::
...
$\bar{X} = \frac{1}{n}\sum_{i}^{n}X_{i}$통계량을 표본평균이라
한다.
:::

::: {.callout-note title="정의"}
확률표본 $\left( X_{1},X_{2},\ldots,X_{n} \right)$의 함수
$S^{2} = \frac{1}{n - 1}\sum_{i}^{n}\left( X_{i} - \bar{X} \right)^{2}$통계량을
표본분산이라 한다.
$\sum_{i}^{n}\left( x_{i} - \bar{x} \right)^{2} = \sum_{1}^{n}{x_{i}^{2} - n\bar{x}}$
:::

::: {.callout-important title="정리"}
표본분산을 최소화 하는 $a = \bar{x}$ 이다.
:::
...
$$ES^{2} = \mathbb{E}\left( \frac{1}{n - 1}\left\lbrack \sum_{i=1}^{n}X_{i}^{2} - n{\overline{X}}^{2} \right\rbrack \right) = \frac{1}{n - 1}\left( n\mathbb{E}\lbrack X_{1}^{2}\rbrack - n\mathbb{E}\lbrack{\overline{X}}^{2}\rbrack \right)$$
...
$\overline{X} = \frac{1}{n}\sum_{i=1}^{n}X_{i}$의
모멘트생성함수는
$M_{\overline{X}}(t) = \left\lbrack M_{X}\left( \frac{t}{n} \right) \right\rbrack^{n}$이다.
:::

::: {.callout-tip title="예제 ①"}
확률표본 $X_{1},\ldots,X_{n}$이 $\mathcal{N}(\mu,\sigma^{2})$
...
코쉬분포의 합, 정규분포의 합, 감마분포의 합이 대표적인 예이다.
:::

::: {.callout-important title="정리"}
확률표본 $X_{1},\ldots,X_{n}$가 다음의 지수족 모집단에서 추출되었다고
하자.
$f(x \mid \theta) = h(x)c(\theta)\exp\left( \sum_{i=1}^{k}w_{i}(\theta)t_{i}(x) \right)$,
이때, 통계량 $T_{1},\ldots,T_{k}$를
$T_{i}(X_{1},\ldots,X_{n}) = \sum_{j=1}^{n}t_{i}(X_{j}),i = 1,\ldots,k$와
같이 정의한다면, 이 통계량들의 결합분포는 다음과 같은 형태의 지수족이
된다.$f_{T}(u_{1},\ldots,u_{k} \mid \theta) = H(u_{1},\ldots,u_{k})\lbrack c(\theta)\rbrack^{n}\exp\left( \sum_{i=1}^{k}w_{i}(\theta)u_{i} \right)$
...
::: {.callout-tip title="예제 ④"}
확률표본 $X_{1},\ldots,X_{n}$이 $Bernoulli(p)$분포에서
추출되었다고 하자.
$T_{1}(X_{1},\ldots,X_{n}) = \sum_{i=1}^{n}X_{i}$는
...
추출되었다고 하자. 표본평균
$\overline{X} = \frac{1}{n}\sum_{i=1}^{n}X_{i}$, 표본분산
$S^{2} = \frac{1}{n - 1}\sum_{i=1}^{n}(X_{i} - \overline{X})^{2}$에 대하여 다음이 성립한다.
...
::: {.callout-important title="보조정리"}

1. 만약 $Z$가 표준정규분포 $\mathcal{N}(0,1)$를 따른다면, $Z^{2}$는
    자유도 1인 카이제곱 분포를 따른다. $Z^{2} \sim \chi_{1}^{2}$

2. 독립인 카이제곱 확률변수 $X_{1},\ldots,X_{n}$가 각각 자유도
    $p_{1},\ldots,p_{n}$을 갖는다고 하자. 확률표본의 합은
    $X_{1} + \cdots + X_{n} \sim \chi_{p_{1} + \cdots + p_{n}}^{2}$을
    따른다.
:::

### 정규 변수들의 선형결합 간의 독립성 판별

::: {.callout-important title="보조정리"}
$X_{j} \sim \mathcal{N}(\mu_{j},\sigma_{j}^{2}),j = 1,\ldots,n$이 서로 독립이라고 하자. 상수 $a_{ij},b_{rj}$에 대해
$i = 1,\ldots,k,r = 1,\ldots,m,j = 1,\ldots,n$, 단 $k + m \leq n$이라
하자. 다음과 같이 선형결합을 정의한다.

$U_{i} = \sum_{j=1}^{n}a_{ij}X_{j},i = 1,\ldots,k$,
$V_{r} = \sum_{j=1}^{n}b_{rj}X_{j},r = 1,\ldots,m$

- 두 확률변수 $U_{i}$와 $V_{r}$는 독립일 필요충분조건은 공분산이 0일
    때이다.
    $U_{i}\bot V_{r} \Longleftrightarrow \text{Cov}(U_{i},V_{r}) = 0$.
    공분산은 다음과 같이 계산된다.
    $\text{Cov}(U_{i},V_{r}) = \sum_{j=1}^{n}a_{ij}b_{rj}\sigma_{j}^{2}$
- 벡터 $(U_{1},\ldots,U_{k})$와 $(V_{1},\ldots,V_{m})$이 상호 독립이기
...
2. 분산분석(ANOVA)에서 독립성 증명: 집단 간 제곱합(SSB)과 집단 내 제곱합(SSW)이 독립임을 보일 때 유용하다.
즉, F-분포의 분자와 분모가 독립이라는 성질을 확인할 때 자주 등장한다.

3. 회귀분석에서 잔차(residual)와 추정량의 독립성: 예측값과 오차항이 독립이라는 성질을 정규 가정하에서 보장할 때 사용된다.

4. 고급 통계 이론 및 추정량의 분산 계산: 선형 추정량의 분산 공식을 도출하거나, 선형 회귀에서 BLUE(최선선형불편추정량) 조건을 분석할 때도 활용된다.

5. 다변량 정규분포의 성질 분석: 독립성 조건을 갖는 여러 선형변환 결과들이 서로 독립이 되는지 판단할 때.
:::
...
quartile($Q_3$)이라 한다. 2사분위수를 중앙값 median이라 한다.

- 사분위 범위 inter-quartile range : ${IQR = Q}_{3} - Q_{1}$

**상자 수염 그림 box whisker plot**

![](images/상자수염그림.png){fig-align="center" width="40%"}
...
$\lim_{n \rightarrow \infty}{P\left( \left| X_{n} - X \right| \geq \epsilon \right) = 0, \text{ for all } \epsilon > 0}$
⬄

$\lim_{n \rightarrow \infty}{P\left( \left| X_{n} - X \right| < \epsilon \right) = 1, \text{ for all } \epsilon > 0}$
:::

::: {.callout-important title="약한 대수의 법칙 (WLLN)"}
$\{ X_{n}\}$은 평균 $\mu$, 분산
$\sigma^{2} < \infty$을 갖는iid 확률변수 시퀀스라 하면
표본평균($\bar{X}_{n}$)은 평균($\mu$)에 확률 수렴한다.
$\bar{X}_{n}\overset{P}{\rightarrow}\mu$

$E\left( \bar{X}_{n} \right) = \mu,V\left( \bar{X}_{n} \right) = \frac{\sigma^{2}}{n}$이므로
...
${g(X}_{n})\overset{P}{\rightarrow}g(X)\text{ where } g \text{ is a continuous function}$.
:::
...
::: {.callout-tip title="예제 ①"}
$\{ X_{n}\}$은 평균 $\mu$, 분산 $\sigma^{2} < \infty$을 갖는
확률표본 시퀀스라 하자. 표본분산
$S_{n}^{2} = \frac{1}{n - 1}\sum_{i}^{n}{(X_{i} - \bar{X}_{n})^{2}}$은
모집단 분산 $\sigma^{2}$의 일치 추정량 이다.
$S_{n}^{2}\overset{P}{\rightarrow}\sigma^{2}$

$S_{n}^{2} = \frac{1}{n - 1}\sum_{i}^{n}{\left( X_{i} - \bar{X}_{n} \right)^{2} = \frac{n}{n - 1}(\frac{1}{n}\sum_{i}^{n}{X_{i}^{2} - \bar{X}_{n}^{2}})\overset{P}{\rightarrow}1\left( E\left( X^{2} \right) - \mu^{2} \right) = \sigma^{2}}$
표본분산은 모분산 일치 추정량 이다.
:::

### 거의 확실한 수렴 ALL sure convergence

::: {.callout-note title="정의"}
$\{ X_{n}\}$ 확률변수 시퀀스, $X$ 확률변수일 때 다음을 만족하면
$X_{n}$은 $X$에 거의 확실한 수렴을 한다고 정의한다.
$P\left( \underset{n \rightarrow \infty}{lim}\left| X_{n} - X \right| < \epsilon \right) = 1, \text{ for all } \epsilon > 0$
:::

::: {.callout-note title="정의"}
평균 $\mu$, 분산 $\sigma^{2} < \infty$을 갖는 확률표본 평균
$\bar{X}_{n}$은 평균에 거의 확실히 수렴한다. (강한 대수의 법칙)

$P\left( \underset{n \rightarrow \infty}{lim}\left| \bar{X}_{n} - \mu \right| < \epsilon \right) = 1, \text{ for all } \epsilon > 0$
:::
...
$\lim_{n \rightarrow \infty}{F_{X}}_{n}(x) = F_{X}(x)\text{ for all } x \in C(F_{X})$.
...
분포 수렴한다.
$\text{if } X_{n}\overset{P}{\rightarrow}X, \text{ then } X_{n}\overset{D}{\rightarrow}X$.
:::

::: {.callout-tip title="예제"}
$X_{1},X_{2},\ldots$은 $U(0,1)$으로부터 확률표본이다. 최대값
$X_{(n)}$의 극한 분포를 구하라.
...
극한 확률밀도함수는 $\lim_{n \rightarrow \infty}{p_{n}(x) = 0} \text{ for all } x$,
...
$\hat{p} = \frac{\sum x_{i}}{n}$의 극한 분포는 중심극한
정리에 의해
$\frac{\sqrt{n}\left( \hat{p} - p \right)}{p(1 - p)}\overset{D}{\rightarrow}N(0,1)$
이다.
:::

::: {.callout-tip title="예제"}
$X_{1},X_{2},\ldots,X_{n}$은 $NB(r,p)$의 확률표본이라 하면
$E\left( X_{i} \right) = \frac{r(1 - p)}{p},V\left( X_{i} \right) = \frac{r(1 - p)}{p^{2}}$
이므로

중심극한정리에 의해
$\frac{\sqrt{n}(\bar{X}_{n} - r(1 - p)/p)}{\sqrt{r(1 - p)/p^{2}}}$
극한분포는 정규분포에 근사한다.
:::

::: {.callout-important title="$\Delta$-method theorem"}
...
$X_{n} = \hat{p} = \frac{\sum x_{i}}{n}$이므로
$\frac{\sqrt{n}\left( \hat{p} - p \right)}{p(1 - p)}\overset{D}{\rightarrow}N(0,1)$

오즈비 $\frac{p}{1 - p}$ 추정한다고 하자.
$X_{n} = \frac{\hat{p}}{1 - \hat{p}}$이고
$g(p) = \frac{p}{1 - p}$이다. ${E(X}_{n}) = \frac{p}{1 - p}$,
$V\left( \frac{\hat{p}}{1 - \hat{p}} \right) \approx \left\lbrack g'(p) \right\rbrack^{2}V\left( \hat{p} \right) = \left\lbrack \frac{1}{(1 - p)^{2}} \right\rbrack^{2}\frac{p(1 - p)}{n} = \frac{p}{n(1 - p)^{3}}$

$\sqrt{n}\left( X_{n} = \frac{\hat{p}}{1 - \hat{p}} - g(p) \right)\overset{D}{\rightarrow}N(0,\frac{p}{n(1 - p)^{3}})$.
:::
...
![](images/시뮬레이션.png){fig-align="center" width="60%"}
...
::: {.callout-tip title="예제"}
전구 수명이 지수분포 $\text{Exponential}(\lambda)$를 따르는다고
...
p_{2} & = P(\text{적어도}t\text{개 이상의 부품이}h\text{시간 이상 작동}) \\
 & = \sum_{k=t}^{c}\binom{c}{k}p_{1}^{k}(1 - p_{1})^{c - k}
\end{matrix}$$
...
- 이들의 평균 $\frac{1}{n}\sum_{i=1}^{n}h(Y_{i})$를 구하면, 이
평균은 $E\lbrack h(Y)\rbrack$에 수렴한다는 것이다. 바로 이것이 대수의
법칙의 핵심입니다.

$Y_{i},i = 1,\ldots,n$이 서로 독립이고 동일한 분포를 따른다고
가정하면(가정이 충족된다는 전제 하에)
$\frac{1}{n}\sum_{i=1}^{n}h(Y_{i})\overset{P}{\rightarrow}E\lbrack h(Y)$이다.
:::

::: {.callout-tip title="예제"}
확률 $p_{2}$는 다음 절차를 통해 시뮬레이션으로 근사할 수 있다.

$j = 1,\ldots,n$에 대해 다음 단계를 수행한다.

1. $X_{1},\ldots,X_{c}$를 서로 독립적으로 생성하되, 각각은 지수분포
    $\text{Exponential}(\lambda)$를 따른다.
2. $X_{i}$ 중에서 $h$시간 이상 작동하는 부품이 $t$개
    이상이면$Y_{j} = 1$, 그렇지 않으면 $Y_{j} = 0$으로 설정한다.

3. 이렇게 정의된 $Y_{j}$는 베르누이분포 $\text{Bernoulli}(p_{2})$를
    따르고, 기대값은 $\mathbb{E}\lbrack Y_{j}\rbrack = p_{2}$이므로
    대수의 법칙에 따라 다음이 성립한다.

$$\frac{1}{n}\sum_{j=1}^{n}Y_{j}\overset{P}{\rightarrow}p_{2}\text{(as}n \rightarrow \infty)$$
...
1. $F(x) = U(0,1)$ : 누적확률밀도함수와 난수생성 일양균일분포 같다고
    한다.
2. $x = F^{- 1}(u)$ 선형합동생성기에 의하여 생성한 난수($(0,1)$
    균일분포)를 이용하여 $x \sim f(x)$을 생성한다.

::: {.callout-tip title="예제"}
지수분포($\beta$)을 따르는 확률변수를 생성하시오.

1. $X \sim exponential(\lambda)$ 이므로
    $F_{X}(x) = \int_{0}^{x}{\frac{1}{\beta}e^{- \frac{x}{\beta}}dx = 1 - e^{- \frac{x}{\beta}}}$
2. $U = 1 - e^{- x/\beta}$ 변환을 하자. $U$의 영역은
    $0 \leq U \leq 1$이고 $X = - \beta ln(1 - U)$
3. $F_{X}^{- 1}(U) = - \beta ln(1 - U)$는 지수분포 $(\beta)$을 갖는다.
:::

::: {.callout-note title="지수분포와 함수관계"}
...
2. 감마분포($\alpha = 1,\beta = 2$) ⬄ 자유도 1인 카이제곱이고 자유도
    1인 카이제곱은 표준정규분포($X$)를 따른다. $\mu + \sigma X$는 평균이
    $\mu$, 표준편차 $\sigma$인 정규분포를 따른다.

::: {.callout-tip title="예제"}
형상모수 $\alpha$, 척도모수 $\beta$인 감마분포를 따르는
확률변수(데이터)를 생성하시오.

1. 일양분포로부터 지수분포($\beta$)을 따르는 확률변수를 생성한다.

2. 생성된 확률변수들의 $\alpha$개 합을 구하면 이 확률변수가 감마분포를
    따른다.
:::
...
2. 다음 조건을 만족하는 함수 $e(x)$는 효율적이다. ① $e(x)$와 $f(x)$는
    확률변수 $X$의 영역에서 유사하다. ② 생성하기 쉬운 확률밀도함수
    $g(x)$에 대하여 $e(x) = ag(x)$이다.

3. $ag(x)$을 생성하여 $f(x)$보다 작으면 채택, 크면 기각하여 $f(x)$을
    따르는 확률변수를 생성하게 된다.

![](images/기각채택 변수생성.png){fig-align="center" width="60%"}

::: {.callout-tip title="예제"}
감마분포($\alpha,\beta$)을 생성하자.

$$f(x) = \frac{1}{\Gamma(\alpha)}x^{\alpha - 1}\beta^{- \alpha}e^{- x/\beta},0 < x < - \infty$$

1. 지수분포를 베이스 분포로 하자. $g(x) = \frac{1}{\beta}e^{- x/\beta}$

2. 상위 경계함수 $e(x) = \frac{a}{\alpha}e^{- x/\alpha}$,
    $a = \frac{\alpha^{\alpha}e^{1 - \alpha}}{\Gamma(\alpha)}$이면,
    $e(x) \geq f(x) \text{ for every } x$ 만족한다.
:::

::: {.callout-tip title="예제"}
베타분포($\alpha = 2,\beta = 4$)을 생성하자.

$$f(x) = 20x(1 - x)^{3},0 < x < 1$$

1. $U(0,1)$을 베이스 분포로 하자. $g(x) = 1,0 < x < 1$

2. $\frac{f(x)}{g(x)} \leq a$을 만족하는 $a$을 구하자.
    $\frac{f(x)}{g(x)} = 20x(1 - x)^{3}$는 $x = 1/4$에서 최대값을
    가지므로 $a = 20(1/4)(1 - 1/4)^{3} = 135/64$

3. 그러므로 $\frac{f(x)}{ag(x)} = \frac{256}{27}x(1 - x)^{3}$이다.

4. 균일분포를 따르는 $U(0,1)_{1},U(0,1)_{2}$을 생성하고 만약
    $U_{2} \leq \frac{256}{27}U_{1}(1 - U_{1})^{3}$이면 $X = U_{1}$을
    채택한다.
:::

::: {.callout-tip title="예제"}
부품 수명이 지수분포($\beta$)을 따른다고 하자. 품질 관리자는
...
$$p_{2} = \sum_{k=t}^{c}\binom{c}{k}p_{1}^{k}(1 - p_{1})^{c - k}$$
:::

::: {.callout-note title="계산"}
만약 생성한 $c$ 개 데이터 중 $h$ 이상인 데이터가 적어도 $t$개
...
![](images/표준정규분포 생성.png){fig-align="center" width="60%"}
...
2. 반복 for $t = 1,2,\ldots,N$: $\begin{matrix}
...
3. 충분히 반복하면, $(x^{(t)},y^{(t)})\overset{d}{\rightarrow}f(x,y)$
...
![](images/이변량정규분포 생성.png){fig-align="center" width="60%"}
...
![](images/박스뮬러 이변량 난수생성.png){fig-align="center" width="60%"}
