...
**【정의】** 통계학에서 분석의 대상이 되는 데이터는 일반적으로 행과 열로 구성된 숫자
...
**【정의】** $X_{1},\ldots,X_{n}$이 어떤 모집단으로부터 크기 n인 임의표본이라고 하자.
...
**【따름정리】** $T = \sum_{i=1}^{n} a_i X_i$ 라고 하자.
...
**【정의】** 확률표본 $\left( X_{1},X_{2},\ldots,X_{n} \right)$의 함수
...
**【정리】**표본분산을 최소화 하는 $a = \bar{x}$ 이다.

**【정리】** 평균 $\mu$, 분산 $\sigma^{2}$인 모집단으로부터 확률표본
...
**【정리】** 확률표본 $X_{1},\ldots,X_{n}$이 모멘트생성함수 $M_{X}(t)$를 갖는
...
**【정리】** 만약 $(X,Y)$가 서로 독립이며 연속형 확률변수이고, 각각의 확률밀도함수가
...
**【정리】** 확률표본 $X_{1},\ldots,X_{n}$가 다음의 지수족 모집단에서 추출되었다고
하자.
...
**【정리】** 확률표본 $X_{1},\ldots,X_{n}$이 $\mathcal{N}(\mu,\sigma^{2})$분포에서
추출되었다고 하자. 표본평균
...
**【보조정리】** $X_{j} \sim \mathcal{N}(\mu_{j},\sigma_{j}^{2}),j = 1,\ldots,n$이 서로 독립이라고 하자. 상수 $a_{ij},b_{rj}$에 대해
...
**【정의】** 이러한 값들은 모두 순서통계량 order statistics 이라 불리며, 자료의 크기
...
**【정의】** 백분위값 percentile은 주어진 값보다 작은 관측값의 비율을 나타내는 중요한
...
**【정리】** $X_{(1)},\ldots,X_{(n)}$을 확률표본 $X_{1},\ldots,X_{n}$의
...
**【정리】** $X_{1},X_{2},\ldots$이 확률변수 $X$에 확률 수렴 하면 연속함수 $g$에
...
**【정리】** $X_{n}\overset{P}{\rightarrow}X,Y_{n}\overset{P}{\rightarrow}Y$ 이면
${X_{n}Y}_{n}\overset{P}{\rightarrow}XY$

**【정의】** 확률분포함수 $F(x;\theta)$를 갖는 모집단으로부터 확률표본
...
**【정의】** $\{ X_{n}\}$ 확률변수 시퀀스, $X$ 확률변수일 때 다음을 만족하면
...
**【정의】** 평균 $\mu$, 분산 $\sigma^{2} < \infty$을 갖는 확률표본 평균
...
**【정의】** $\{ X_{n}\}$ 확률변수 시퀀스, $X$ 확률변수이고 $F_{X_{n}},F_{X}$ 각각
...
**【정리】** $X_{n}$ 확률변수 시퀀스가 $X$ 확률변수에 확률 수렴하면 $X_{n}$은 $X$에
분포 수렴한다.
