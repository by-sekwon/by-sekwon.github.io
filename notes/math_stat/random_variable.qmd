...
::: {.callout-note title="이산형 확률변수 (Discrete Random Variable)"}
...
::: {.callout-note title="연속형 확률변수 (Continuous Random Variable)"}
...
::: {.callout-note title="히스토그램 작성 절차"}
...
::: {.callout-note title="커널 밀도 추정량 (KDE)"}
...
**최적 bandwidth (Silverman's rule of thumb):**
...
::: {.callout-caution title="쌍봉 분포에서 단순 평균 추론의 한계"}
...
::: {.callout-note title="분포 형태와 중심 척도 관계"}
| 분포 형태 | 특징 | 예 |
|-----------|------|-----|
...
::: {.callout-note title="치우침 측정 지표"}
**평균 기반 왜도 (Moment-based)**
$$\text{Skewness} = E\left[\left(\frac{X-\mu}{\sigma}\right)^3\right] = \frac{\mu_3}{\sigma^3}$$
정규분포의 왜도 = 0, 지수분포의 왜도 = 2 (강한 우측 치우침)

**중앙값 기반 척도 (Pearson)**
- First Skewness: $\text{skew} = \dfrac{\text{mean} - \text{mode}}{\text{std}}$
- Second Skewness: $\text{skew} = \dfrac{3(\text{mean} - \text{median})}{\text{std}}$

**사분위 기반 Bowley's Skewness** (이상치 영향 적음)
...
::: {.callout-important title="실증적 법칙 (Empirical Rule)"}
