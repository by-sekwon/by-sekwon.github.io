...
$T(\mathbf{X})$가 모수 $\theta$에 대한 충분통계량이라면 $\theta$에 대한 모든 추론은 확률표본 $\mathbf{X}$의 전체 값이 아니라 $T(\mathbf{X})$의
...
$X_{1},\ldots,X_{n}$을 모수 $\theta \in (0,1)$인 베르누이
...
::: {.callout-tip title="예제 ②"}
$X_{1},\ldots,X_{n}$을 $\sigma^{2}$가 알려진
$N(\mu,\sigma^{2})$ 정규분포를 따르는 확률표본이라 하자.
$T(\mathbf{X}) = \sum X_{i}/n$는 $\mu$에 대한 충분통계량이다.

$$f(\mathbf{x}|\mu) = \prod_{i=1}^{n}(2\pi\sigma^{2})^{- 1/2}\exp\left( - \frac{(x_{i} - \mu)^{2}}{2\sigma^{2}} \right)$$
...
$f(\mathbf{x}) = \prod_{i=1}^{n}f(x_{i}) = \prod_{i=1}^{n}f(x_{(i)})$가
성립하므로 위의 정리에 따라 순서통계량이 충분통계량이 된다.
:::

::: {.callout-important title="분해 정리, Factorization Theorem"}
$f(\mathbf{x}|\theta)$를 표본
...
$$f(\mathbf{x}|\mu) = (2\pi\sigma^{2})^{- n/2}\exp\left( - \sum_{i=1}^{n}(x_{i} - \overline{x})^{2}/(2\sigma^{2}) \right)\exp\left( - n(\overline{x} - \mu)^{2}/(2\sigma^{2}) \right)$$

$$h(\mathbf{x}) = (2\pi\sigma^{2})^{- n/2}\exp\left( - \frac{1}{2\sigma^{2}}\sum_{i=1}^{n}(x_{i} - \overline{x})^{2} \right)$$
...
::: {.callout-tip title="예제 ⑤"}
$X_{1},\ldots,X_{n}$을 $N(\mu,\sigma^{2})$ 정규분포를 따르는
확률표본이라 하자. $T_{1}(\mathbf{x}) = \overline{X}$,
$T_{2}(\mathbf{x}) = S^{2} = \frac{1}{n - 1}\sum_{i=1}^{n}(X_{i} - \overline{X})^{2}$은
...
::: {.callout-important title="정리"}
$f(x \mid \theta) = h(x)c(\mathbf{\theta})\exp\left( \sum_{i=1}^{k}w_{i}(\mathbf{\theta})t_{i}(x) \right)$
지수족 분포로부터의 충분통계량은
$T(\mathbf{X}) = \left( \sum_{j=1}^{n}t_{1}(X_{j}),\sum_{j=1}^{n}t_{2}(X_{j}),\ldots,\sum_{j=1}^{n}t_{k}(X_{j}) \right)$이다.
:::

#### 최소 충분통계량

충분통계량은 모수 $\theta$에 대한 정보를 표본에서 손실 없이 요약할 수
있는 통계량이다. 그런데 모든 충분통계량이 "작거나 간단한"
...
::: {.callout-tip title="예제 ⑦"}
확률표본 $X_{1},\ldots,X_{n}$이 $U(\theta,\theta + 1)$에서
...
$T'(\mathbf{X}) = \left( \sum_{i=1}^{n}X_{i},\sum_{i=1}^{n}X_{i}^{2} \right)$
또한 최소 충분통계량이다.
:::
...
::: {.callout-note title="정의"}
어떤 통계량 $T(\mathbf{X})$에 대해 확률분포족 $f(t|\theta)$가 있을 때, 이 분포족을 완비라고 부른다. 완비란, 모든 $\theta$에 대해
...
::: {.callout-tip title="예제 ②"}
확률표본 $X_{1},\ldots,X_{n}$이 $U(0,\theta)$에서
추출되었다고 하자. $T(\mathbf{X}) = max(x_{i}) = x_{(n)}$은 모수
$\theta$에 대한 완비 충분 통계량이다.
:::

::: {.callout-important title="Basu's Theorem"}
만약 $T(\mathbf{X})$가 완비하고 최소 충분
...
$f(x|\theta) = h(x)c(\theta)\exp\left( \sum_{j=1}^{k}w(\theta_{j})t_{j}(x) \right)$.
다음 $T(\mathbf{X})$는 완비통계량이다.
$T(\mathbf{X}) = \left( \sum_{i=1}^{n}t_{1}(X_{i}),\sum_{i=1}^{n}t_{2}(X_{i}),\ldots,\sum_{i=1}^{n}t_{k}(X_{i}) \right)$
:::

::: {.callout-important title="Minimal Complete Statistic"}
만약 최소 충분통계량이 존재한다면, 모든
완비통계량도 최소 충분통계량이다.
:::

::: {.callout-tip title="예제 ③"}
$X_{1},\ldots,X_{n}$을 $exp(\theta)$, 지수분포(지수족)를
따르는 확률표본이라 하자.
...
::: {.callout-tip title="예제 ④"}
$X_{1},\ldots,X_{n}$을 $N(\mu,\sigma^{2})$,
...
::: {.callout-tip title="예제 ①"}
$NB(r = 3,p)$, 음이항분포로부터 $X_{1} = 2$ 관측되었다면
우도함수는 $P_{p}(X = 2) = \binom{4}{2}p^{3}(1 - p)^{2}$이다.
:::

::: {.callout-note title="우도 원리"}
표본점 $\mathbf{x},\mathbf{y}$가 다음 조건을 만족한다고
하자.
...
관찰했을 경우와 $y$를 관찰했을 경우 추론 결과가 "일정한
관계"를 가져야 한다고 요구한다. 반드시 동일할 필요는
...
::: {.callout-tip title="예제 ①"}
$X \sim \text{Binomial}(n,p)$일 때, 성공 횟수 $x$를 관찰한
...
::: {.callout-note title="정의"}
모집단 확률분포함수 $f(x;\theta),\theta \in \Omega$의 확률표본에서 얻은 통계량 $T = T\left( X_{1},X_{2},\ldots,X_{n} \right)$이 모수 추정에
...
- 표본 적률: $m_{1}' = E(X) = \bar{X}$,
$m_{2}' = E\left( X^{2} \right) = \frac{1}{n}\sum X_{i}^{2}$

- 방정식: $\mu = \bar{X}$,
$\sigma^{2} + \mu^{2} = \frac{\sum X_{i}^{2}}{n}$

- 평균 추정량: $\overset{\hat{}}{\mu} = \bar{x}$

- 분산 추정량:
$\overset{\hat{}}{\sigma^{2}} = \frac{1}{n}\sum X_{i}^{2} - {\bar{X}}^{2} = \frac{1}{n}\sum\left( X_{i} - \bar{X} \right)^{2}$
:::

::: {.callout-tip title="예제 ③"}
모집단 $U(0,\theta)$으로부터 확률표본
$\left( X_{1},X_{2},\ldots,X_{n} \right)$이다. 적률방법으로  추정량
$\widehat{\theta}$ 구하라.
...
- $\bar{X} = \frac{\theta}{2}$ 이므로 적률에 의한 추정량은
$\overset{\hat{}}{\theta} = 2\bar{X}$이다.

  -------------------------------------------------------------------------------------------------------------
  추정량 $\overset{\hat{}}{\theta} = 2\bar{X}$은 불편 추정량은
  ($E\left( 2\bar{X} \right) = 2\theta \neq \theta$)아니지만 일치 추정량이다. 【정리】 만약
...
::: {.callout-tip title="예제 ④"}
모집단 $Gamma(\alpha,\beta)$으로부터 확률표본
...
- 방정식: $\alpha\beta = \bar{X}$,
$\alpha\beta^{2} + \alpha\beta^{2} = \frac{\sum X_{i}^{2}}{n}$

- 추정량: $\overset{\hat{}}{\alpha} = \frac{n\bar{X}}{n\sum\left( X_{i} - \bar{X} \right)^{2}}$,
$\overset{\hat{}}{\beta} = \frac{\sum\left( X_{i} - \bar{X} \right)^{2}}{n\bar{X}}$

불편 추정량은 아니지만 $\bar{X}$는 $\alpha\beta$의 일치
...
우도함수 최대화 하는 $\theta$를 최대우도 maximum likelihood
추정량 이라 한다.
...
::: {.callout-note title="로그 우도함수"}
우도 함수는 항상 0보다 크므로 우도함수 최대화 하는
$\theta$ 계산 ⬄ 로그 우도함수 최대화 하는 $\theta$ 계산
:::

::: {.callout-tip title="예제 ①"}
어느 지역의 암 환자 비율 $p$을 추정하려고 한다. 모수 $p$인 베르누이 확률밀도함수로부터 확률표본을 추출하였다고 하자.
...
::: {.callout-tip title="예제 ②"}
빼빼로 중량이 $N(\mu,\sigma^{2})$을 따른다고 하자. 중량
...
그러므로 $\overset{\hat{}}{\mu} = \bar{X},{\overset{\hat{}}{\sigma}}^{2} = \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{n}$이다.
:::

::: {.callout-tip title="예제 ③"}
모집단 확률분포 $U(0,\theta)$, 확률표본에서 MLE을 구하시오.
...
- 미분:
$\frac{\partial l}{\partial\theta} = \sum_{i}^{n}{sgn(x_{i} - \theta)} = 0,\text{ where } \text{sgn}(t) = \left\{ \begin{array}{r}
 - 1,t < 0 \\
0,t = 0 \\
1,t > 0
\end{array} \right.\ $
...
::: {.callout-tip title="예제 ⑤"}
$N(\mu,1), \text{ where } \mu > 0$을 따르는 확률표본을 이용하여 $\mu$에
대한 MLE 구하라.
...
::: {.callout-tip title="예제 ⑥"}
$B(n,p)$을 따르는 확률표본을 이용하여 $n$에 대한 MLE
...
결론적으로 $(1 - p)^{n} = \prod_{i=1}^{n}(1 - x_{i}z)$. 구간
...
::: {.callout-important title="invariance property"}
$f(x;\theta),\theta \in \Omega$을 따르는
...
::: {.callout-tip title="예제 ①"}
$N\left( \mu,\sigma^{2} \right)$에서
$\overline{\theta} = (\mu,\sigma^{2})$ MLE는
$\overset{\hat{}}{\mu} = \bar{X},{\overset{\hat{}}{\sigma}}^{2} = \frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{n}$.

invariance property에 의해 표준편차
$\sigma = \sqrt{\sigma^{2}}(\tau(\theta))$ MLE는
$\overset{\hat{}}{\sigma} = \sqrt{\frac{\sum\left( x_{i} - \bar{x} \right)^{2}}{n}}$이다.
:::

::: {.callout-tip title="예제 ②"}
$B(p)$에서 확률표본 $(X_{1},X_{2},\ldots,X_{n})$이다.
$V(X)$의 MLE 구하라.
...
::: {.callout-note title="정의"}
모수 $\theta$에 대한 MLE ${\overset{\hat{}}{\theta}}_{mle}$은 일치 추정량이다.
:::
...
::: {.callout-note title="사후확률"}
$\pi(\theta)$ 모수 사전 prior 확률밀도함수,
...
::: {.callout-note title="bayes estimator 베이지안 추정량 정의"}
...
::: {.callout-tip title="예제 ②"}
$N\left( \theta,\sigma^{2} \right)$에서 추출한 확률표본
$\overline{X} = \left( X_{1},X_{2},\ldots,X_{n} \right)$을 이용하여
...
::: {.callout-note title="MSE"}
추정량 $W$와 모수 $\theta$에 대해, 평균제곱오차(Mean Squared
...
::: {.callout-note title="편향"}
추정량 $W$ 의 편향은 ${Bias}_{\theta}(W) = E_{\theta}W - \theta$ 으로 정의된다.
...
::: {.callout-tip title="예제 ①"}
$f(x;\theta) \sim N(\mu,\sigma^{2})$에서 표본크기 $n$인
...
::: {.callout-tip title="예제 ②"}
$f(x;\theta) \sim U(\theta,\theta + 1)$에서 표본크기 $n$인
확률표본을 추출하였다. 추정량 $\bar{X}$가 편의 추정량 임을 보이고 MSE 구하라.
...
::: {.callout-tip title="예제 ③"}
$f(x;\theta) = \frac{1}{\theta}e^{- x/\theta},0 < x$ 에서
...
::: {.callout-tip title="예제 ④"}
$f(x;p) = p^{x}(1 - p)^{1 - x},x = 0,1$ 베르누이 분포에서
...
::: {.callout-note title="상대효율"}
${eff(\overset{\hat{}}{\theta}}_{1},{\overset{\hat{}}{\theta}}_{2}) = \frac{{V(\overset{\hat{}}{\theta}}_{2})}{V({\overset{\hat{}}{\theta}}_{1})}$, 추정량 ${\overset{\hat{}}{\theta}}_{2}$에 대한
...
::: {.callout-tip title="예제 ①"}
$f(x;\theta) \sim U(0,\theta)$에서 표본크기 $n$인 확률표본을
...
"최고의 MSE 추정량"은 존재하지 않는다. 이는 후보 추정량의
...
::: {.callout-tip title="예제 ①"}
$f(x;\theta) \sim B(n,p)$에서 표본크기 $n$인 확률표본을
...
::: {.callout-important title="Cramér--Rao Lower Bound, CRLB"}
확률밀도함수 $f(x|\theta)$의
...
::: {.callout-note title="Fisher Information"}
확률밀도함수 $f(x|\theta)$가 지수족을 따르다면
...
$E_{\lambda}\left( \left( \frac{\partial}{\partial\lambda}\log\prod_{i=1}^{n}f(X_{i}|\lambda) \right)^{2} \right) = - nE_{\lambda}\left( \frac{\partial^{2}}{\partial\lambda^{2}}\log f(X|\lambda) \right)$
...
### Rao-Blackwell 정리 & MVUE

::: {.callout-note title="MVUE 정의"}
$f(x;\theta)$ 에서 표본크기 $n$인 확률표본
...
::: {.callout-important title="rao-blackwell theorem"}
추정량  $\overset{\hat{}}{\theta}$는 모수
...
::: {.callout-note title="ancillary statistics"}
모수 $\theta$에 의존하지 않는 통계량
...
::: {.callout-tip title="예제"}
$f(x;\theta) \sim B(\theta = p)$에서 표본크기 $n$인 확률표본
...
::: {.callout-note title="완비성 completeness"}
충분 통계량
$T\left( \overline{x} \right) \sim f(t;\theta)$을 갖는다고 하자. 만약
$E_{\theta}\left( g(T) \right) = 0\text{ for all } \theta$가
$P_{\theta}\left( g(T) = 0 \right) = 1\text{ for all } \theta$을 포함하면
$T\left( \overline{x} \right)$는 완비 통계량이다.
:::

::: {.callout-tip title="예제"}
$f(x;\theta) \sim B(\theta = p)$에서 표본크기 $n$인 확률표본을
추출하였다. 충분 통계량 $\sum X_{i}$은 완비 통계량임을 보이시오.

- $T = \sum X_{i} \sim B(n,p)$ 이다.

- $0 = E_{p}\left( g(T) \right) = \sum_{t}^{n}{g(t)\binom{n}{t}p^{t}(1 - p)^{n - t} = (1 - p)^{n}}\sum_{t}^{n}{g(t)\binom{n}{t}{(\frac{p}{1 - p})}^{t}\text{ for all } 0 < p < 1}$
...
::: {.callout-tip title="예제"}
$f(x;\theta) \sim U(0,\theta)$에서 표본크기 $n$인 확률표본
...
::: {.callout-important title="basu theorem"}
최소 충분 통계량이고 완비 통계량 $T\left( \overline{x} \right)$는 다른 모든 보조 통계량과 독립이다.
:::
...
::: {.callout-important title="lehmann and scheffe theorem"}
$f(x;\theta),\theta \in \Omega$ 에서
...
::: {.callout-tip title="예제"}
$f(x;\theta) = \frac{1}{\theta},0 < x < \theta$에서 표본크기
...
::: {.callout-tip title="예제"}
$f(x;\theta) = B(\theta = p)$에서 표본크기 $n$인 확률표본을
추출하였다. MVUE 구하라.
...
::: {.callout-tip title="예제"}
$f(x;\theta = \lambda) = e^{- \lambda}\frac{\lambda^{x}}{x!}$에서 표본크기 $n$인 확률표본을 추출하였다. MVUE 구하라.
...
::: {.callout-tip title="예제"}
$f(x;\theta = \mu) = N\left( \mu,\sigma^{2} \right), \text{ where } \sigma^{2} \text{ is known}$에서 표본크기 $n$인 확률표본을 추출하였다. 모수 $\theta = \mu$MVUE 구하라.
...
임을 보였다. 그러므로 $\frac{p(1-p)}{n}$에 대한 MVUE을 구하기 위하여 분포를 알고 있는 $Y = \sum x_{i} \sim B(n,p)$ 함수를 이용하는 것이 적절하다.
...
::: {.callout-tip title="예제"}
$f(x;\theta) = \left( \frac{1}{\theta} \right)e^{- x/\theta} \sim exponential(\theta)$에서 표본크기 $n$인 확률표본을 추출하였다. $V(X) = \theta^{2}$ MVUE 구하라.
