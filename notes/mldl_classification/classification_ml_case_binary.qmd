...
::: {.callout-note}
## Breast Cancer Wisconsin 데이터셋 개요

| 항목 | 내용 |
|------|------|
| **목적** | 유방 종양 악성(malignant=0)/양성(benign=1) 이진 분류 |
| **표본 수** | n = 569 |
| **변수 수** | p = 30 (모두 연속형) |
| **변수 유형** | mean / error / worst 유형의 세포핵 형태 통계 |
| **예시 변수** | mean radius, mean perimeter, mean area 등 |

클래스가 균형에 가까운 **표준 이진분류 벤치마크** 데이터이다.
:::
...
1. **Train/Test 분리 먼저**: 테스트는 학습에 절대 사용 금지 — 75% 학습 / 25% 테스트
2. **층화 분할(stratify=y)**: 클래스 비율 유지 (악성:양성 = 212:357 불균형 보정)
3. **표준화는 Pipeline 내에서**: train에만 `fit` → test에는 `transform`만 (데이터 누수 방지)

SVM(RBF 커널)과 k-NN은 거리/내적 기반 → **변수 스케일에 매우 민감** → 표준화 필수
:::
...
::: {.callout-important}
## kNN vs SVM 임계값 전략 비교 결론

| 전략 | 모델 | t | FP | FN | Recall | FPR | 특징 |
|------|------|---|----|----|--------|-----|------|
| **Youden/FPR≤0.02** | SVM | 0.593 | 1 | 1 | 0.981 | 0.011 | 균형형 최적점 |
| **비용기반** (c_FN=10) | kNN | 0.254 | 11 | **0** | **1.000** | 0.122 | 놓침 제로, 오경보 큼 |
| FPR≤0.02 | kNN | 0.423 | 1 | 3 | 0.943 | 0.011 | 오경보 ↓, 놓침 ↑ |

- **일반 균형 성능**: SVM 우세 — FP=1, FN=1로 동시에 낮음
- **악성 절대 놓침 금지**: kNN 비용기반 — FN=0 달성, 단 FP=11 감수

**핵심**: 임계값은 운영 정책(비용 구조)에 의해 결정된다. 동일 모델도 임계값에 따라 운영 특성이 완전히 달라진다.
:::
...
::: {.callout-warning}
## 대규모 불균형 데이터에서 모델 선택 전략

이 데이터는 표본이 매우 커서 RBF SVM / kNN을 그대로 적용하면 학습이 매우 느리다.

| 전략 | 권장 방법 |
|------|---------|
| **속도 최적화** | Linear SVM + calibration |
| **kNN/SVM** | 학습 샘플 일부만 사용 (서브샘플링) |
| **앙상블** | RandomForest, GradientBoosting — 불균형에 강건 |
:::
...
|------|------|
| **총 거래** | 284,807건 |
| **변수** | 31개 (V1~V28: PCA 변환, Time, Amount, Class) |
| **정상(0)** | 284,315건 (99.827%) |
| **사기(1)** | 492건 (**0.173%**) — 극심한 불균형 |

V1~V28은 PCA 변환으로 원래 변수 해석 불가. Accuracy만으로는 "모두 정상" 예측도 99.8% → **PR-AUC, Recall, 비용기반 임계값**이 핵심 지표.
:::
