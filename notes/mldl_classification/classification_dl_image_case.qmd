...
| 항목 | 내용 |
|------|------|
| **이미지 크기** | 32×32 픽셀, RGB 3채널 (NHWC 포맷) |
| **학습 데이터** | 50,000장 |
| **테스트 데이터** | 10,000장 |
| **클래스 수** | 10개 (균형 분포) |
| **픽셀 범위** | 0~255 (uint8 원시값) |

**클래스 라벨**: 0=airplane, 1=automobile, 2=bird, 3=cat, 4=deer, 5=dog, 6=frog, 7=horse, 8=ship, 9=truck
:::
...
#### 전처리: 정규화 0~1 + train/valid 분리 + augmentation

::: {.callout-tip}
## 전처리 3단계 원칙

1. **픽셀 정규화** (딱 1번만): `float32` 변환 후 `/ 255.0` → 0~1 범위 → 최적화 수치 안정성 확보
2. **층화 분리** (stratify): train 80% / val 20% — 클래스 비율 유지로 공정한 성능 비교
3. **데이터 증강** (훈련시에만): RandomFlip + RandomRotation + RandomZoom → 모델 내부에서 적용, 검증/테스트에는 미적용

**핵심 원칙**: 정규화 중복 금지 · 검증/테스트에 증강 미적용 · 재현성 위한 seed 고정
:::
...
| 단계 | 구성 | 목적 |
|------|------|------|
| **합성곱 블록 1** | Conv2D(32, 3×3, ReLU) + MaxPooling | 저수준 특징(엣지, 색상) 추출 |
| **합성곱 블록 2** | Conv2D(64, 3×3, ReLU) + MaxPooling | 중간 수준 특징 추출 |
| **합성곱 블록 3** | Conv2D(128, 3×3, ReLU) + MaxPooling | 고수준 추상 특징 추출 |
| **분류 헤드** | Flatten → Dense(128, ReLU) → Dropout(0.3) → Dense(10, softmax) | 클래스 확률 출력 |

**학습 설정**: Adam(lr=1e-3) · SparseCategoricalCrossentropy · EarlyStopping(patience=6) · ReduceLROnPlateau
:::
...
**속도 개선**
- `WIDTH_MULT`로 채널 폭 축소 → FLOPs 감소
- 블록당 합성곱 1회로 경량화
- `tf.data` cache/prefetch → 입력 파이프라인 병목 완화
- Mixed precision → GPU 연산·메모리 효율 개선

**일반화 개선**
- BatchNormalization → 학습 안정화
- L2 가중치 감쇠 + Dropout → 과적합 완화
- 데이터 증강(RandomFlip/Rotation/Zoom) 적용
- **Flatten 대신 GAP** → 파라미터 수 대폭 감소

**핵심 차이**: Baseline은 Flatten, Improved는 Global Average Pooling(GAP) 사용
:::
...
| 모델 | 테스트 정확도 | macro F1 | weighted F1 | 비고 |
|------|------------|---------|------------|------|
| **Baseline CNN** | **0.7531** | **0.7531** | **0.7531** | 안정적, 전반적으로 고른 성능 |
| Improved CNN | 0.4845 | 0.4493 | 0.4493 | 성능 저하 — 설정 재검토 필요 |

**결론**: 현재 설정에서 Improved CNN은 개선이 아닌 **성능 저하** — BN/L2/GAP 자체가 나쁜 것이 아니라, 모델 폭 축소 + 규제 강도 + 학습 스텝 제한의 복합 조건으로 **과소적합(underfitting)** 발생 가능성이 큼. 실무적으로는 현 시점에서 **Baseline CNN 채택이 더 합리적**.
:::
...
::: {.callout-caution}
## Improved CNN 성능 저하 원인 분석

Improved CNN이 Baseline보다 낮은 성능을 보인 이유:

- **모델 폭 축소** (`WIDTH_MULT=0.75`): 채널 수 감소로 표현력 제한
- **학습 스텝 제한** (`STEPS_PER_EPOCH=200`): 에폭당 학습 데이터 40,000장 중 일부만 사용
- **강한 규제** (BN + L2 + Dropout): 작은 모델에 과도한 규제 → 과소적합
- **증강 강도**: 32×32 소형 이미지에서 회전/줌이 오히려 정보 손실 유발 가능

**개선 방향**: WIDTH_MULT=1.0 유지 + STEPS_PER_EPOCH=None + 규제 강도 완화
:::
