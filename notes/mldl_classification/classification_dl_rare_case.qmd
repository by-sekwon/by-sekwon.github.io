...
::: {.callout-note}
## Credit Card Fraud 데이터셋 개요

| 항목 | 내용 |
|------|------|
| **총 거래 수** | 284,807건 |
| **변수 수** | 31개 (V1~V28: PCA 변환, Time, Amount, Class) |
| **정상(Class=0)** | 284,315건 (약 99.827%) |
| **사기(Class=1)** | 492건 (약 **0.173%**) — 극심한 불균형 |
| **목표** | $P(Y=1 \mid X=x)$ 추정 후 운영 임계값으로 의사결정 |

**핵심 주의**: 단순 정확도(Accuracy)로 평가하면 "모두 정상" 예측만으로도 99.8%가 나온다. PR-AUC, Top-K 회수율, 비용 기반 임계값이 핵심 평가 지표이다.
:::
...
1. **층화 분리(stratify=y)**: 사기 비율을 train/test에서 동일하게 유지
2. **선택적 스케일링**: Amount, Time만 StandardScaler 적용 (V1~V28은 PCA 완료)
3. **누수 방지**: `fit_transform(X_train)` / `transform(X_test)` — 테스트 정보가 전처리에 반영되면 성능 과대평가
4. **순서 주의**: ColumnTransformer 출력 시 변환 컬럼(Amount, Time)이 앞, remainder 컬럼이 뒤에 배치
:::
...
::: {.callout-warning}
## 극심한 불균형에서 학습이 실패하는 이유

사기 비율 ≈ 0.17%에서 가중치 없이 학습하면:

- 모델이 학습 초기에 “거의 전부 0(정상)”으로 예측해도 손실이 작다
- 사기를 맞히는 방향으로 학습 신호가 약해져 수렴 실패

**해결**: `pos_weight = n_neg / n_pos ≈ 577` → 사기 1건 = 정상 577건의 손실 비중

| 구분 | 내용 |
|------|------|
| **장점** | 구현 단순, PR-AUC/Recall 개선 경우 많음 |
| **한계** | 가중치 너무 크면 확률 과대추정(calibration 문제), FP 증가 가능 |

→ 학습 후 운영에서 **임계값(threshold) 정책 재설정 필수**
:::

시드(seed) 고정이 필요한 이유: 딥러닝 학습 과정에는 난수가 많이 들어간다(가중치 초기화, 미니배치 셔플, 드롭아웃 등). 시드를 고정하지 않으면 같은 코드라도 실행할 때마다 성능이 달라질 수 있다. 그래서 시드 고정은 실험을 과학적으로 비교하기 위한 최소 조건이다.

불균형 가중치(sample_weight)가 필요한 이유: `pos_weight = n_neg / n_pos`는 “양성 1개가 음성 몇 개에 해당하는가”를 맞춰주는 대표적인 균형화 방식이다. 현재 출력값이 약 577이면, 사기 1건을 정상 약 577건 정도의 중요도로 보도록 설정한 셈이다.
...
| 방법 | 원리 | 장점 | 단점/주의 |
|------|------|------|---------|
| **MLP + 가중 BCE** | 양성 손실에 가중치 | 구현 간단, 안정적 | Calibration 문제 가능 |
| **MLP + Focal Loss** | 어려운 샘플에 집중 | PR-AUC 높음, Recall 우수 | gamma/alpha 튜닝 필요 |
| **오토인코더(AE)** | 정상만 학습 후 재구성오차 | 레이블 없이 사용 가능 | 레이블 있으면 성능 제한 |
:::
...
#### MLP + Focal Loss (불균형에서 Recall 향상에 유리)
...
| 파라미터 | 역할 |
|---------|------|
| $\gamma$ (기본값 2.0) | 쉬운 샘플의 기여도 억제 강도 — $\gamma$ 클수록 어려운 샘플에 더 집중 |
| $\alpha$ (기본값 0.25) | 클래스 가중치 — 양성(사기) 손실 비중 조절 |

**핵심**: 모델이 자신 있게 맞히는(pt가 큰) 정상 거래의 손실 기여를 줄이고, 사기 거래처럼 어려운 샘플에 집중 — 불균형 데이터에서 Recall 향상에 효과적
:::
...
| 방법 | ROC-AUC | PR-AUC(AP) | Recall@Prec≥0.80 | P@100 | R@100 |
|------|---------|-----------|-----------------|-------|-------|
| **MLP + Focal Loss** | **0.9857** | **0.8303** | **0.8163** | **0.80** | **0.8163** |
| MLP + weighted BCE | 0.9795 | 0.6584 | 0.6837 | 0.73 | 0.7449 |
| Autoencoder | 0.9475 | 0.5902 | 0.1531 | 0.62 | 0.6327 |

**핵심 결론**: 불균형 데이터에서 ROC-AUC는 과대평가될 수 있음 → **PR-AUC(AP)와 운영 지표(Top-K, 정밀도 제약)가 핵심** → Focal Loss가 일관되게 우수
:::
...
| 정책 | 핵심 질문 | 선택 기준 | 적합 상황 |
|------|---------|---------|---------|
| **Top-K** | 검토 인력이 K건 처리 가능할 때 | TP 최대화 | 조사 물량 고정 |
| **비용 최소** | $c_{FN}$·FN + $c_{FP}$·FP 최소 | 총 비용 최소 | 미탐·오탐 비용 명확 시 |
| **정밀도 제약** | Precision ≥ p₀ 유지하면서 Recall 최대 | Recall 최대화 (Precision 하한 조건) | FP 제한이 우선인 운영 |

**불균형 데이터 임계값 선택 핵심**: 0.5를 기본값으로 쓰면 안 됨 — 운영 목적에 맞는 임계값을 검증셋에서 명시적으로 선택해야 함
:::
