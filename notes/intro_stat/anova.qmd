...
| 용어 | 설명 |
|------|------|
| **절대실험** | 요인을 조작하지 않고 관찰·조사로 현재 상태 파악 |
| **비교실험** | 요인을 변화시켜 반응의 변화 분석 → ANOVA 대상 |
| **요인(Factor)** | 연구자가 통제하는 변수 (예: 사료 종류, 수온) |
| **처리(Treatment)** | 요인 수준의 조합 (2수준 × 2수준 = 4처리) |
| **대조 처리(Control)** | 다른 처리의 효과를 비교하는 기준 (플라세보 포함) |
| **실험 단위** | 처리를 무작위 할당받는 물리적 개체 |
| **반복(Replication)** | 동일 처리를 서로 다른 독립 실험 단위에 적용 |
| **실험 오차** | 동일 처리를 받은 실험 단위 간 반응 변동 |
:::

**절대실험과 비교실험**: 절대실험은 관심 있는 대상의 현재 상태나 특성을 파악하기 위해 요인을 조작하지 않고 관찰이나 조사를 통해 정보를 얻는 방식이다. 반면 비교실험은 관심 있는 현상에 영향을 미칠 수 있는 요인을 변화시키고, 그에 따른 반응의 변화를 분석하는 실험이다. 분산분석(ANOVA)은 이러한 비교실험을 분석하는 대표적인 방법이다.

**설계된 실험 (designed experiment)**: 미리 정해진 틀 안에서 집단을 관찰·측정·평가하는 연구를 뜻한다. 연구자는 실험 중 해당 틀의 요소들을 통제하여, 관심 있는 집단 간에 타당한 비교가 가능하도록 데이터를 수집한다. 이러한 통제를 통해 통계적 추론이 가능해진다.

**요인 (factors)**: 물고기 사료(기존 사료A, 새로 개발된 사료B)와 수온(20도, 25도)에 따른 몸무게 증가 효과를 보기 위하여 어항 4개, 물고기 12마리를 랜덤하게 선택하여, 각 어항에 3마리 물고기 배정 후, (사료×온도) 어항 넣은 후 2주 후에 몸무게를 측정하였다.
...
**처리 (treatments) 및 설계 형태**: 처리는 요인의 수준을 조합하여 구성된다. 위 실험에서는 2수준(사료) × 2수준(수온) = 4개의 처리가 가능하다.
...
**대조 처리 (control treatment)**: 다른 처리들의 효과를 비교하는 기준이 되는 특별한 처리 유형이다. 대조 처리가 특히 중요한 경우는 세 가지다.
...
**실험 단위 (experimental unit)**: 처리를 무작위로 할당받는 물리적 개체, 혹은 처리 집단 중 하나에서 무작위로 선택된 실험 대상이다. 예를 들어, 물고기 몸무게 실험에서는 "어항"이 실험 단위이다.

**반복 (replication)**: 처리가 한 실험 단위에 할당되면 이를 해당 처리의 한 번의 반복이라고 한다. 일반적으로는 각 처리를 여러 실험 단위에 무작위로 배정하여, 각 처리에 대해 여러 번의 반복을 수행한다.

**실험 오차 (experimental error)**: 동일한 처리를 받고 동일한 실험 조건에서 관측된 실험 단위들 간 반응의 변동을 말한다. 실험 오차가 0이 되지 않는 이유에는 다음이 포함된다.
...
**Placebo 효과**: 플라세보 효과는 실제로 치료 성분이 없거나 효과가 없는 처치를 받았음에도 불구하고, 환자가 치료를 받았다고 믿기 때문에 증상이 호전되는 현상을 말한다. 이러한 플라세보 효과를 배제하기 위해 이중 블라인드(double blind) 실험을 적용한다.
...
| 방법 | 원리 | 효과 |
|------|------|------|
| **블록화 (Blocking)** | 유사 단위를 묶어 블록 내 균질성 확보 | 블록 간 변동을 오차에서 분리 |
| **공변량 (Covariate)** | 반응변수와 관련된 보조변수를 모형에 포함 | 공변량이 설명하는 변동을 오차에서 제거 |
| **반복 (Replication)** | 각 처리를 여러 독립 단위에 적용 | 오차 추정 정확도 향상, 검정력 증가 |
| **무작위화** | 처리 배정을 무작위로 수행 | 미지의 요인에 의한 편향 방지 |
:::

**블록화를 통한 실험오차 줄이기**: 블록화(blocking)는 실험 단위 집단 내에서 중요한 특성 차이가 클 때, 실험 오차 분산을 줄이는 데 효과적인 방법이다. 반응변수에 영향을 줄 수 있는 특성에 따라 실험 단위를 비슷한 것끼리 묶어 블록을 만들면, 각 블록 내부는 균질해지고 전체적으로는 다양한 특성을 포함할 수 있다.

**공변량을 이용한 변동성 줄이기**: 공변량(covariate)은 반응변수와 관련이 있는 변수로, 실험 오차의 변동을 줄이는 데 활용된다. 공변량이 되기 위해서는 세 가지 조건을 만족해야 한다. 첫째, 반응변수와 관계가 있어야 하고, 둘째, 측정이 가능해야 하며, 셋째, 처리에 의해 영향을 받지 않아야 한다.
...
|------|------|
| **오차 독립** | $e_{ij}$는 서로 독립 |
| **오차 정규성** | $e_{ij} \sim N(0, \sigma^2)$ |
| **등분산성** | 모든 처리 집단의 분산 동일 |
| **$\mu$** | 모집단 총평균 |
| **$\alpha_i$** | 요인 $i$수준의 처리 효과 (주효과) |
| **$\mu_i = \mu + \alpha_i$** | 수준 $i$ 모집단 평균 |
| **검정 가설** | $H_0: \mu_1 = \mu_2 = \cdots = \mu_k$ |
:::
...
**추정량 (MVUE)**: MVUE(최소분산 불편추정량)는 다음과 같이 정의된다.
...
| 방법 | 특징 | 엄격성 | 주요 용도 |
|------|------|:------:|---------|
| **Fisher's LSD** | 분산분석 후 쌍별 t-검정 | 낮음 | 탐색적 비교 |
| **Tukey HSD** | Studentized 범위 분포 활용 | 중간 | 자연과학 (가장 많이 사용) |
| **SNK / Duncan** | 간격(step) 수에 따라 임계값 달라짐 | 중간 | 순차적 비교 |
| **Dunnett** | 대조군 vs. 각 실험군만 비교 | 중간 | 대조군 포함 실험 |
| **Scheffé** | 모든 가능한 선형 대비 허용 | 높음 | 일반적 선형 비교 |

**엄격성**: 낮음(많은 유의 차이 탐지) → 높음(보수적, 적은 유의 차이 탐지)
:::
...
**Fisher's Least Significant Difference**: Fisher의 최소유의차(LSD) 방법은 분산분석에서 전체 평균이 같다는 귀무가설을 기각한 이후, 구체적으로 어떤 집단 평균이 서로 다른지를 알아보기 위해 고안된 사후(pairwise) 비교 절차이다.

$$LSD_{ij} = t_{\frac{\alpha}{2}}\sqrt{MSE\left( \frac{1}{n_{i}} + \frac{1}{n_{j}} \right)}$$

요인 수준 $i$와 $j$의 평균 차이가 위의 값 이상이어야 유의하다고 판단한다.

**Tukey HSD (honestly significant difference) procedure**: Tukey의 W 절차는 다중비교에서 개별 비교 오류율을 통제할 때 발생하는 주요 단점을 해결하기 위해 제안된 방법이다. **Studentized 범위 분포(Studentized range distribution)**를 이용한 방법이다. 보수적인(귀무가설 기각하지 않음) 방법으로 자연 과학에서 가장 많이 이용한다.
...
**Student-Newman-Keuls(SNK) ⟺ Duncan Multiple range test**: SNK 절차는 Studentized 범위 통계량을 사용하지만, 비교되는 평균들 간의 간격(steps)의 수에 따라 서로 다른 임계값을 적용한다.

$$W_{r} = q_{\alpha}(r,\nu)\sqrt{\frac{MSE}{n}}$$

**Dunnett's Procedure**: Dunnett(1955) 방법은 대조군과 각 실험군의 평균을 비교할 때 실험 단위 제1종 오류율을 통제하는 절차이다.

$$D = d_{\alpha}(k,v)\sqrt{\frac{2MSE}{n}}$$

여기서 $k = t - 1$(t는 요인 개수), $n_{c} = n_{1} = \cdots = n_{t - 1} = n$, 그리고 $v$는 오차변동의 자유도이다.

**Scheffe's S method**: Scheffé(1953)가 제안한 절차는 t개의 모집단 평균 사이에서 가능한 모든 비교를 수행할 수 있는 보다 일반적인 방법이다.
...
::: {.callout-note icon=false}
## 분꽃 품종별 꽃잎 길이 일원 ANOVA 결과

| 품종 | 평균 | 표준편차 | F-통계량 | 유의확률 |
|------|:---:|:-------:|:-------:|:-------:|
| setosa^a | 1.46 | 0.174 | 1180 | < 0.001 |
| versicolor^b | 4.26 | 0.470 | | |
| virginica^c | 5.55 | 0.552 | | |

귀무가설 기각: 품종 간 꽃잎 길이는 통계적으로 유의한 차이가 있다.
Tukey 사후검정: virginica^c > versicolor^b > setosa^a (모든 쌍 유의)
:::
...
| 구분 | CRD (완전임의배치) | RCBD (무작위 완전블록) |
|------|----------------|-------------------|
| **설계 원리** | 모든 실험 단위에 처리를 무작위 배정 | 블록 내에서만 처리 무작위 배정 |
| **블록** | 없음 | 있음 (동질 단위 묶음) |
| **모형** | $y_{ij} = \mu + \alpha_i + e_{ij}$ | $y_{ij} = \mu + \alpha_i + \beta_j + e_{ij}$ |
| **자유도 (오차)** | $n - k$ | $(t-1)(b-1)$ |
| **적용 조건** | 실험 환경이 균일 | 블록 요인이 반응변수에 영향 |
| **정밀도** | 낮음 (블록 변동 미분리) | 높음 (블록 변동 분리 제거) |
:::
...
**블록효과 검정**: 상대효율은 블록화 설계가 완전임의배치설계에 비해 처리 평균을 얼마나 더 정밀하게 추정할 수 있는지를 나타내는 척도이다.
...
| 요인 | F-통계량 | 유의확률 | 결론 |
|------|:-------:|:-------:|------|
| cut (처리) | 159.1 | < 0.001 | 연마 품질별 가격 차이 유의 |
| color (블록) | 278.9 | < 0.001 | 색상별 가격 차이 유의 |
| **상대효율 (RE)** | **50.05** | — | RCBD가 CRD보다 약 50배 정밀 |
:::
...
| 상호작용 | 의미 | 시각적 특징 |
|---------|------|-----------|
| **없음** | A 효과가 B 수준에 관계없이 일정 | 평균도표의 선들이 평행 |
| **있음** | A 효과가 B 수준에 따라 달라짐 | 평균도표의 선들이 교차 또는 발산 |

**⚠️ 상호작용이 유의하면**: 주효과만 해석하면 오류. 각 수준 조합의 셀 평균으로 해석해야 한다.
:::
...
**모평균 추정**: $\widehat{\mu} = {\overline{y}}_{...} = \frac{1}{abn}\sum_{ijk}y_{ijk}$, 표본 총평균
...
| 요인 | 내용 | 유의확률 | 결론 |
|------|------|:-------:|------|
| **origin** (생산국가) | 일본 > 유럽 > 미국 순 연비 | < 0.001 | 유의 |
| **cylinders** (실린더 수) | 실린더 많을수록 연비 낮아짐 | < 0.001 | 유의 |
| **origin × cylinders** (상호효과) | 생산국가별 실린더 수에 따른 연비 변화 패턴 다름 | < 0.001 | 유의 |

→ 상호작용이 유의하므로 주효과만 해석하면 오류. 생산국가·실린더 조합별 셀 평균을 함께 해석해야 한다.
:::
...
**목적**: 공변량(covariate)을 통제하여 처리 효과를 보다 정확하게 추정

**공변량의 3가지 조건**

| 조건 | 내용 |
|------|------|
| **반응변수와 관련** | 공변량이 종속변수 변동을 설명할 수 있어야 함 |
| **측정 가능** | 실험에서 실제로 수치로 측정되어야 함 |
| **처리 영향 없음** | 처리(요인) 배정에 의해 공변량 값이 바뀌면 안 됨 |

**ANOVA vs ANCOVA**

| 구분 | ANOVA | ANCOVA |
|------|-------|--------|
| 모형 | $y = \mu + A_i + B_j + (AB)_{ij} + e$ | $y = \mu + A_i + B_j + (AB)_{ij} + \gamma x + e$ |
| 공변량 | 없음 | 있음 ($x$) |
| 오차 분산 | 큼 | 작음 (공변량 설명 후) |
:::
...
| 요인 | 유의확률 | 결론 |
|------|:-------:|------|
| **diet** (식이요법) | < 0.001 | 유의: no fat(121) > low fat(105) |
| **time** (운동시간) | < 0.001 | 유의: 시간 길수록 맥박 증가 |
| **diet × time** (상호작용) | 유의 | 식이요법별 시간에 따른 맥박 패턴 다름 |
| **rest_pulse** (공변량) | 비유의 | 운동 전 맥박을 통제 후 효과 없음 |

**결론**: 저지방 식이요법이 운동 후 시간이 지남에 따라 맥박 상승을 억제하는 효과가 있다.
:::
