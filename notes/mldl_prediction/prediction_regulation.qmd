...
![](images/prediction_regulation_concept.png){fig-align="center" width="60%"}
...
| 상황 | 문제 |
|------|------|
| **다중공선성** | $X^\top X$가 불안정 → 계수 추정 분산 급증 |
| **고차원** ($p \geq n$) | OLS가 불안정하거나 해가 유일하지 않음 |
| **과적합** | 훈련 데이터에는 잘 맞지만 새 데이터에서 오차 증가 |

규제 회귀는 "데이터 적합도"에 더해 "계수의 복잡도"에 벌점을 부여하여, 계수를 0 방향으로 수축시키는 방식이다.
$$\hat{\beta}(\lambda) = \arg\min_{\beta} \|y - X\beta\|^2 + \lambda P(\beta), \quad \lambda \geq 0$$
:::

규제 회귀는 "데이터 적합도"에 더해 "계수의
...
규제의 핵심 목적은 **분산(variance)을 줄이기 위해 편향(bias)을 일부 허용**하는 데 있다.
...
테스트 오차(MSE)는 $\lambda$에 대해 "그릇(U) 모양"을 보이며, **최적의 $\lambda$**가 존재한다. $\lambda$가 너무 크면 편향이 커져 테스트 오차가 다시 증가한다.
:::
...
수축하는 것이지, "모든 $x_{ij} = 0$"일 때의 평균 수준을
...
s는 "계수 크기에 대한 예산"이며, Ridge는 L2-예산 안에서
...
::: {.callout-note}
## Ridge의 장점: 분산 감소 > 편향 증가

Ridge의 장점은 편향-분산 트레이드오프에서 나온다.

- $\lambda = 0$ (OLS): 편향 없음, 분산 큼
- $\lambda$ 증가: 계수 수축 → **분산 크게 감소, 편향 약간 증가**
- 중간 $\lambda$에서: 분산 감소 이득 > 편향 증가 손해 → **테스트 MSE 감소**
- 너무 큰 $\lambda$: 편향이 과도하게 커져 MSE 다시 증가

흥미롭게도, $\lambda = 0$ (OLS)의 높은 분산 때문에 테스트 MSE가, $\lambda = \infty$인 영모형(null model)의 MSE와 거의 비슷할 정도로 커질 수 있다. 그러나 중간 $\lambda$에서는 MSE가 훨씬 낮아질 수 있다.
:::
...
바꾼 "계산 가능한 대안"으로 이해할 수 있다. 특히 Lasso는
계수를 0으로 만들므로 subset selection과 더 가깝다.

#### 왜 Lasso는 0을 만드는가: 기하학적 직관

::: {.callout-note}
## Ridge vs Lasso: 제약 영역의 기하학

| 방법 | 제약 영역 | 접점 특성 |
|------|----------|-----------|
| **Ridge** | 원(구) — 모서리 없음 | 접점이 축 위에 걸리기 어려움 |
| **Lasso** | 마름모(다면체) — 뾰족한 꼭짓점 | 접점이 **축 위 꼭짓점**에 걸리기 쉬움 → $\beta_j = 0$ 발생 |

RSS 등고선(타원)이 Lasso의 꼭짓점에 걸릴 때 해당 계수가 정확히 0이 된다. 이것이 Lasso가 자동으로 변수 선택을 수행하는 이유이다.
:::

#### 수축 형태 비교: Ridge vs Lasso (직교 특수 경우)

::: {.callout-important}
## Soft-thresholding: Lasso가 0을 만드는 수식

서로 직교하는 단순한 경우, 두 방법의 수축 방식 차이가 식으로 명확해진다.

- **Ridge**: 계수를 비율로 연속적으로 줄임 — $\hat{\beta}_j^R = \frac{1}{1+\lambda} y_j$
- **Lasso**: $\frac{\lambda}{2}$만큼 당기고, 절댓값이 작으면 **0으로 절단**

$$\hat{\beta}_{j}^{L} = \begin{cases}
y_j - \lambda/2 & (y_j > \lambda/2) \\
y_j + \lambda/2 & (y_j < -\lambda/2) \\
0 & (|y_j| \leq \lambda/2)
\end{cases}$$

이 **soft-thresholding** 형태가 "Lasso가 왜 0을 만드는가"를 수식으로 설명해준다.
:::
...
| 방법 | 패널티 | 변수 선택 | 특징 |
|------|--------|-----------|------|
| **Ridge (L2)** | $\lambda\sum\beta_j^2$ | ❌ 모두 포함 | 분산 안정화, 계수 연속 수축 |
| **Lasso (L1)** | $\lambda\sum|\beta_j|$ | ✅ 계수=0 가능 | 희소성, 상관 강하면 불안정 |
| **Elastic Net** | L1 + L2 혼합 | ✅ 부분적 | 희소성 + 안정성 동시 달성 |
...
::: {.callout-note}
## Elastic Net CV 표준 절차

1. **표준화**: 규제 회귀는 스케일에 민감 → X를 평균 0, 분산 1로 표준화(절편은 패널티 제외)
2. **$\alpha$ 후보 그리드**: $\alpha \in \{0.1, 0.3, 0.5, 0.7, 0.9\}$ 등
3. **각 $\alpha$마다 $\lambda$ 그리드**(로그 스케일) + K-fold CV 수행
4. **전체 후보 중 CV 오차 최소** $(\hat{\alpha}, \hat{\lambda})$ 선택
5. **선택된 값으로 전체 훈련 데이터에 재적합** → 최종 모형

보고 시: $\lambda_\min$ (CV 최소) vs $\lambda_{1\text{SE}}$ (1-SE 규칙) 중 어떤 것을 택했는지 명시. $\lambda_{1\text{SE}}$는 성능 손실 없이 더 단순한 모형을 제공하는 경우가 많다.
:::
...
Elastic Net(Ridge, Lasso 포함) 계수는 "예측을 위한 최적화 결과"이며, OLS처럼 표준오차/유의확률 해석을 그대로 붙이기 어렵다.

안전한 해석 방향:
- 선택된 변수(0이 아닌 계수)
- 계수의 **상대적 크기와 방향** (표준화 기준)
- 예측 성능(CV/테스트 오차)
...
::: {.callout-tip}
## Elastic Net을 기본 선택으로 쓸 조건

다음 조건 중 하나라도 강하면, Lasso 단독보다 Elastic Net이 기본 선택이 된다.

- 설명변수들 간 **상관이 강한 경우** (변수군 존재)
- 원-핫 인코딩 등으로 유사 더미변수 집합이 큰 경우
- $p$가 크고(고차원) "희소성 + 안정성"을 동시에 원할 때
...
1.11 정도는 "과도하게 크지도, 0에 가깝지도 않은" 중간 수준
규제로 보는 게 자연스럽다.

**Ridge 계수 해석(중요: 표준화된 계수)**

현재 파이프라인에 StandardScaler()가 들어가 있으므로, 계수 크기는
"원 단위"가 아니라 표준화된 단위(1 표준편차 변화) 기준이다.
...
- **최적 $\lambda \approx 1.11$**: 중간 수준 규제 — 과도하지도, 0에 가깝지도 않음
- **가장 중요한 변수**: weight (-5.02) > model_year (+2.77) — 무게 증가 시 연비 감소, 연식 최신화 시 연비 증가
- **다중공선성**: displacement(+2.18)와 horsepower(-1.43)는 방향이 반대인데, 이는 두 변수가 강하게 상관되어 있어 Ridge가 계수를 **분산시켜 안정화**한 결과 (인과적 해석 주의)
- **테스트 성능**: $R^2 \approx 0.82$, $RMSE \approx 3.07$ — 양호한 예측력
:::
...
교차검증이 선택한 $\lambda$가 그리드의 최솟값(0.001) 이다. 이는 "규제를 거의 걸지 않는 쪽(OLS에 가까운 쪽)"이
검증오차를 가장 줄였다는 뜻이다.
...
::: {.callout-note}
## Lasso 분석 결론: 변수 선택 미발생

- **최적 $\lambda = 0.001$ (그리드 최솟값)**: CV가 "규제를 거의 걸지 않는 쪽"을 선택 → 현재 데이터에서는 강한 희소화가 도움이 되지 않음
- **Non-zero coefficients: 9/9**: Lasso이지만 변수 선택이 발생하지 않아 사실상 OLS/Ridge와 비슷한 해
- **테스트 성능**: RMSE≈3.063, $R^2 \approx 0.816$ — **Ridge와 거의 동일**

변수 수가 9개로 적고 과적합 위험이 크지 않아, Lasso가 추가 이점을 주지 못한 결과이다.
:::

**Lasso에서 "진짜 변수선택"이 나오는지 확인하는 방법**

현재는 $\lambda$가 너무 작게 선택되어 희소화가 안 된 상태이므로, 아래 중
하나를 해보면 교육적으로도 좋다. $\lambda$ 범위를 더 넓히기(특히 더 크게): `alphas = np.logspace(-4, 1, 400)` 또는 `np.logspace(-3, 2, 400)` 또는 `LassoCV(alphas=None)`으로 두고 sklearn이 자체 그리드를 잡게 하기
...
alpha가 커질수록 계수들이 0으로 "붙는다" 계수경로는 x축이
...
즉, 경로 그림은 "어떤 변수가 먼저 살아남고(끝까지 남고),
...
선택(계수=0)을 강하게 만들기 어렵다. 즉 "예측 안정성(분산
...
"조건부 효과가 규제 강도에 따라 재배치된다"로 해석하는 것이
안전하다.
...
강하기 때문이다. Ridge 성향이 강하면 계수는 "작아지기"는
하지만 0이 되기 어렵다.

::: {.callout-note}
## Elastic Net 분석 결론 요약

| 항목 | alpha_min | alpha_1se |
|------|-----------|-----------|
| $\alpha$ (l1_ratio) | 0.1 (Ridge 성향 90%) | 동일 |
| $\lambda$ | 0.00320 | 0.221 |
| Non-zero 변수 수 | 9개 (전부) | 8개 (acceleration 제거) |

- **best l1_ratio = 0.1**: Ridge 성향이 매우 강함 → 변수 선택이 거의 안 됨
- **displacement 부호 변화** (alpha_min: +2.16 → alpha_1se: -0.82): 다중공선성 변수군에서 규제 강도에 따라 계수가 재배치 — **인과적 해석 금지**
- 변수를 더 줄이려면: `l1_ratios = [0.5, 0.7, 0.9, 1.0]` 등으로 Lasso 성향 증가
:::

**변수 개수를 더 줄이고 싶다면(Elastic Net에서)**

Elastic Net에서도 Lasso처럼 더 줄이려면 다음 중 하나를 하면 된다.
`l1_ratios` 후보를 더 Lasso 쪽으로 두기: `[0.5, 0.7, 0.9, 1.0]` 또는 `np.linspace(0.5, 1.0, 6)`

혹은 "예측 최적" 대신 `alpha_1se`를 기본 선택으로 삼기(지금처럼), 또는 목표 non-zero 개수(예: 5개)를 정해서 그에 맞는 alpha를 고르는 방식(희소성 제약식 관점)도 가능하다.
