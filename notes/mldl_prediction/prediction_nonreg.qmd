...
이 장의 핵심은 모형을 비선형으로 바꾸는 것이 아니라, **입력 X를 적절한 함수로 변환하여** 여전히 선형회귀 프레임 안에서 비선형성을 표현하는 것이다.

$$Y = \beta_0 + \sum_{j=1}^{p} f_j(X_j) + \varepsilon, \quad \mathbb{E}[\varepsilon] = 0$$

$f_j(\cdot)$를 어떻게 잡느냐가 방법론을 갈라놓는다.

| 방법 | $f_j(\cdot)$ 형태 | 특징 |
|------|-----------------|------|
| 다항회귀 | $X^d$ 항 | 전역 다항식 |
| 계단함수 | 구간 지시함수 | 불연속 |
| 회귀 스플라인 | 조각 다항식 | 매끄러운 연결 |
| 스무딩 스플라인 | 규제된 자연 3차 스플라인 | λ로 복잡도 제어 |
| 국소회귀 | 목표점 주변 가중 회귀 | 국소 적합 |
| GAM | 변수별 독립 비선형 함수 | 가법 구조 |

**일관된 원칙**: 복잡도를 키우면 훈련오차는 줄지만 과적합 위험이 커지므로, **검증오차(교차검증)나 정보기준, 규제(패널티)**를 통해 복잡도를 통제한다.
:::
...
형태(가속/포화/굽음)를 보이는 경우가 많다. 이때 "모형을
...
| 문제 | 설명 |
|------|------|
| **경계 요동(런지 현상)** | 구간 끝부분에서 곡선이 과도하게 흔들림 |
| **다중공선성 심화** | $X, X^2, \ldots, X^d$ 항들이 강하게 상관 → 계수 분산 증가 |
| **외삽 불안정** | 학습 구간 밖에서 예측값 급격히 발산 |
| **차원 폭발** | 다변량에서 $m(p,d) = \binom{p+d}{d}$ → $d=2$만 돼도 $O(p^2)$ |

차수 d는 개별 계수의 유의성이 아니라 **교차검증**을 통해 선택하는 것이 원칙이다.
:::
...
"모형을 조금 더 유연하게 만든다"는 수준을 넘어, 특징공간의
...
설명변수 개수의 결정은 단순히 "원래 변수 p를 얼마나 넣을
것인가"라기보다, 실제로는 "그중 몇 개를 다항 확장 대상
...
"추가했을 때 검증오차가 의미 있게 감소하는가"를 기준으로
...
결과는 본질적으로 "구간별 평균이 얼마나 다른가"로 해석되며,
...
| 유형 | 제약 수 | 자유도 (매듭 1개) | 특징 |
|------|---------|-----------------|------|
| **조각별 3차 (무제약)** | 0 | 8 | 매듭에서 불연속 허용 |
| **C⁰ 제약** | 1 | 7 | 함수값 연속, 기울기 꺾임 가능 |
| **3차 스플라인 (C² 제약)** | 3 | 5 | 함수값·1차·2차 도함수 모두 연속 |
| **선형 스플라인 (C⁰, d=1)** | 1 | — | 직선 구간, 매듭에서 꺾임 |

일반적으로 내부 매듭이 $K$개이면, **3차 스플라인의 자유도 = K+4**
:::
...
매듭에서 꺾임이 생긴다. 차수 d의 스플라인은 "차수 d의
...
"조각별 차수 d 다항식을, (그리고 가능하다면 그 1차부터
...
SSE만 최소화하면 $g(x_i) = y_i$인 과적합이 발생한다. 이를 방지하기 위해 **매끈함에 대한 패널티**를 추가한다.

$$\sum_{i=1}^{n}(y_i - g(x_i))^2 + \lambda \int (g''(t))^2 \, dt$$

| 항 | 역할 |
|----|------|
| $\sum(y_i - g(x_i))^2$ | 데이터 적합도 (손실) |
| $\lambda \int (g''(t))^2 dt$ | 함수의 거칠기에 대한 패널티 |

$g''(t)$는 함수의 "거칠기"를 측정한다. **직선의 $g'' = 0$**, 즉 완전히 매끈하다.

| $\lambda$ | 효과 |
|-----------|------|
| $0$ | 패널티 없음 → 훈련점 정확히 보간 (과적합) |
| 중간 | 데이터와 매끈함의 균형 |
| $\infty$ | 완전히 매끈 → 선형 최소제곱 직선 |
:::

이를 최소화하는 $g(x)$는 $x_1, \ldots, x_n$에 매듭을 둔 **자연 3차 스플라인**임이 알려져 있다.
...
맥락에서 보았던 "손실+패널티(loss+penalty)" 형태를 취한다.
...
"거칠기"를 벌점으로 제어하면서 결정된다. $\lambda$가
...
나타내며, 주황색으로 표시된 점들은 $x_{0}$ 주변의 "이웃"
...
1. $x_0$에 가장 가까운 훈련점들 중 비율 $s = k/n$에 해당하는 점들을 모은다.
2. 각 점에 가중치 $K_{i0} = K(x_i, x_0)$ 부여 ($x_0$에서 멀수록 가중치 감소 → 0)
3. 가중 최소제곱 회귀를 적합한다.
$$\sum_{i=1}^{n} K_{i0}(y_i - \beta_0 - \beta_1 x_i)^2 \text{ 최소화}$$
4. 적합값: $\hat{f}(x_0) = \hat{\beta}_0 + \hat{\beta}_1 x_0$

**핵심 선택**: (1) span/대역폭 $s$ → 유연성 결정, (2) 커널 $K$ 형태, (3) 국소 모형 차수
:::

**주의**: $p > 3\text{~}4$가 되면 $x_0$ 근처 훈련 관측치가 매우 적어져 성능이 나빠질 수 있다.
...
- 각 $X_j$에 대해 비선형 함수 $f_j$를 자동으로 적합 — 여러 변환을 일일이 시도할 필요 없음
- 가법 구조 덕분에 다른 변수를 고정한 채 **각 $X_j$의 효과를 개별 해석** 가능
- 함수 $f_j$의 매끈함을 자유도로 요약 가능

**한계**

- 모형이 **가법 구조로 제한** → 변수 간 복잡한 상호작용을 놓칠 수 있음
- 상호작용이 필요하면 $X_j \times X_k$ 항이나 $f_{jk}(X_j, X_k)$를 수동 추가 필요

**위치**: GAM은 **선형모형과 완전한 비모수 모형 사이의 유용한 절충안**이다. 더 유연한 모형이 필요하면 랜덤 포레스트·부스팅으로 확장한다.
:::
...
::: {.callout-note}
## 사례분석 결과 비교: mpg ~ horsepower

| 방법 | 설정 | 훈련 $R^2$ |
|------|------|-----------|
| 선형 회귀 | — | 0.606 |
| 다항회귀 | $d = 8$ (CV 선택) | 0.694 |
| 계단함수 | 5 bins | 0.660 |
| 계단함수 | 10 bins | 0.697 |
| 기저함수 | knots = 4개 | ~0.700 |
| 회귀 스플라인 | df = 11 | 0.708 |
| 스무딩 스플라인 | s = 1200 | 0.802 |
| 국소회귀 | frac = 0.1 | 0.711 |
| GAM (1변수) | s(horsepower) | 0.707 |
| GAM (2변수) | s(horsepower) + s(weight) | **0.758** |

비선형 회귀 방법들이 선형 회귀($R^2 = 0.606$) 대비 일관되게 성능 향상을 보임. **2변수 GAM**이 가장 높은 $R^2$를 달성.
:::
...
#### 계단함수 $5bins:R^{2} = 0.66,10bins:R^{2} = 0.697$
