...
뉴턴은 ["]{dir="rtl"}왜 달은 하늘에 떠 있는 반면, 사과는 땅으로
떨어질까?"라는 질문을 통해 만유인력의 법칙을 정립하고 중력과 중력
가속도를 발견했다.

한편, 미분은 ["]{dir="rtl"}변화"로 정의된다. 기존 상태와 변화된 상태는
...
$f'(a)$가 존재하면, 점 $x = a$에서 함수 $f(x)$는 미분 가능하다고 한다.

함수 $f(x)$가 정의역 전체에서 미분 가능하, 함수 f(x)는 미분 가능
함수이다.
...
$h$가 0으로 가까워질수록 평균 변화율은 접선의 기울기에 점점 가까워진다.
...
하지만, 연속이라고 해서 항상 미분 가능한 것은 아니다. 예를 들어, 절대값
함수 가능하지 않다.
...
$\frac{d}{dx}\lbrack x^{n}\rbrack = nx^{n - 1}$,
...
오차 제곱합 $S(\beta_{0},\beta_{1})$을 $\beta_{0}$와 $\beta_{1}$에 대해
