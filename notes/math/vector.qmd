...
$\underset{¯}{b} = (b_{1},b_{2},b_{3})$의 외적![붙여넣은
동영상.png](media/image13.png){width="1.659909230096238in"
height="1.2968044619422572in"}
$\underset{¯}{a} \times \underset{¯}{b}$는 다음과 같이 계산한다.
...
1. 각도가 $\theta = 90^{o} = \pi/2$이면 두 벡터는 직교 orthogonal 한다.

2. 각도가 $\theta = 0^{o}$이면 두 벡터는 정렬 aligned 되어 있다.

3. 각도가 $\theta = 180^{o} = \pi$이면 두 벡터는 역정렬 anti-aligned
    되어 있다.

4. 각도가 $\theta > 90^{o} = \pi/2$이면 두 벡터의 각은 둔각 obtuse,
