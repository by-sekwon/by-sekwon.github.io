...
::: {.callout-note icon=false}
## 일반화 선형모형(GLM)의 3가지 구성요소

| 구성요소 | 내용 |
|---------|------|
| **확률분포** | 종속변수는 정규·이항·포아송 등 지수분포족에 속한다고 가정 |
| **선형예측자** | 독립변수들의 선형 결합 $\eta = X\beta$ |
| **연결함수** | 종속변수의 기댓값과 선형예측자를 연결 (정규→항등, 이항→로짓, 포아송→로그) |

고전적 회귀분석은 GLM의 특별한 경우(정규분포 + 항등 연결함수)이다.
:::
...
| 단계 | 내용 |
|------|------|
| **① 모형 설정** | 이론·선행연구 바탕으로 목표변수·예측변수 설정 |
| **② 데이터 전처리** | 산점도 탐색, 변수 변환, 결측치·이상치 처리 |
| **③ 모형 추정** | OLS로 잔차제곱합 $\sum(y_i - \hat y_i)^2$ 최소화 |
| **④ 유의 변수 선택** | t-검정으로 유의한 변수 식별, 필요 시 LASSO 등 활용 |
| **⑤ 다중공선성 진단** | VIF로 예측변수 간 높은 상관관계 확인 |
| **⑥ 모형 진단 및 활용** | 잔차로 선형성·등분산성·독립성·정규성 가정 점검 |
:::
...
::: {.callout-note icon=false}
## 회귀모형 4가지 오차 가정 $\underset{¯}{e} \sim MNV(\underset{¯}{0},\sigma^{2}I)$

| 가정 | 내용 | 위배 시 문제 |
|------|------|------------|
| **선형성** | $E(Y\|X)$가 $X$의 선형 함수 | 계수 편의, 예측 오류 |
| **독립성** | 오차항이 서로 독립 | 표준오차 과소추정 (시계열 특히 중요) |
| **정규성** | $e_i \sim N(0, \sigma^2)$ | t·F-검정 신뢰성 저하 |
| **등분산성** | 모든 $X$ 값에서 오차 분산 동일 | 표준오차 왜곡 (이분산성) |
:::
...
모형: $Y = \beta_{0} + \beta_{1}D_{\text{대졸}} + \beta_{2}D_{\text{대학원}} + u$
:::
...
::: {.callout-tip icon=false}
## 빅데이터 환경에서의 범주형 변수 처리 방법

| 방법 | 적용 조건 | 특징 |
|------|---------|------|
| **더미변수** | 수준 10~20개 이하 | 해석 용이, 차원 증가 |
| **Target Encoding** | 수준 수백 이상 | 종속변수 평균으로 대체, 데이터 누설 주의 |
| **Hash Encoding** | 수준 수백~수천 | 고정 차원, 범주 충돌 가능 |
| **Embedding** | 수준 수천 이상 | 딥러닝, 범주 간 유사성 반영 |

해석 목적 → 더미 변수 / 예측 목적 → target encoding, 임베딩, 해싱
:::
...
::: {.callout-note icon=false}
## OLS 추정치 주요 성질 (GAUSS-MARKOV Theorem)

**BLUE (Best Linear Unbiased Estimator)**: 모든 선형·불편 추정량 중 최소 분산을 가진다.

| 성질 | 내용 |
|------|------|
| **불편성** | $E(\hat{\underset{¯}{b}}) = \underset{¯}{b}$ |
| **분산** | $V(\hat{\underset{¯}{b}}) = \sigma^2(X'X)^{-1}$ |
| **추정 분산** | $\hat V(\hat{\underset{¯}{b}}) = MSE(X'X)^{-1}$ |
| **적합치** | $\hat{\underset{¯}{y}} = H\underset{¯}{y}$, Hat 행렬 $H = X(X'X)^{-1}X'$ |
| **잔차** | $\underset{¯}{r} = (I-H)\underset{¯}{y}$, 잔차 분산 $V(\underset{¯}{r}) = \sigma^2(I-H)$ |
:::
...
| 진단 | 기준 | 처리 |
|------|------|------|
| **① 함수관계** | 직선·곡선·무관계 등 패턴 확인 | 비선형이면 변수 변환 고려 |
| **② 이상치** | 표준화 잔차 $\pm 2$ 벗어나는 관측치 | 고전적: 삭제 / 빅데이터: 중요 정보로 활용 |
| **③ 영향치** | X축 범위를 벗어난 관측치 (Hat 통계량) | 순수 영향치(결정계수 왜곡) vs 이상 영향치(기울기 왜곡) |
:::
...
| 항목 | 결과 |
|------|------|
| **결정계수 ($R^2$)** | 0.741 → 예측변수가 주택가격 변동의 74.1% 설명 |
| **유의하지 않은 변수** | age (p값 > 0.05) |
| **주요 유의 변수** | lstat(-), rm(+), ptratio(-), nox(-) 등 |
| **다중공선성 경고** | Notes [2] 경고 출력 → VIF 진단 필요 |

age 예측변수를 제외하고는 모든 예측변수는 유의하다. 다중공선성 문제 발생 경고 → 추가 진단 필요.
:::
