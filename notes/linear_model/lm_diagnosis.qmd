...
![pasted-image.pdf](media/image3.png){width="2.3761286089238847in"
height="1.7081069553805774in"}
...
데이터에 설정된 회귀모형이 완전하다면 (목표변수 관측값- 추정 모형에 의해
적합된 값) 차이인 잔차에는 어떤 패턴도 있어서는 안된다. 즉 백색잡음,
정규분포를 따라야 한다.
...
예측변수 DIS 상관계수 부호와 회귀계수 부호

+-----------------------------------------------------------------------+
| \# \-\-- 4) 최종 모형 적합 \-\--                                      |
|                                                                       |
| selected = \[\'CHAS\', \'NOX\', \'RM\', \'PTRATIO\', \'B\',           |
| \'LSTAT\'\] \# 선택된 변수 리스트                                     |
|                                                                       |
...
본 연구에서는 목표변수와 설명변수 간의 상관계수가 매우 유의하게
나타났으며, 이는 곧 선형 함수관계가 존재함을 의미한다. 따라서 선형성
가정은 충족되었고, 앞서 언급한 것처럼 굳이 별도의 검정을 수행할 필요는
없다는 결론에 도달한다.
...
|                                                                       |
| print(f\"\[Harvey--Collier\] t = {t_stat:.3f}, p-value =              |
| {p_val:.4g}\")                                                        |
|                                                                       |
...
이상치가 발생할 수 있기
때문이다.![pastedGraphic.pdf](media/image6.png){width="4.303011811023622in"
height="1.2250929571303586in"}
...
BP, White는 ["]{dir="rtl"}이분산 있음" 쪽으로 강한 신호.

GQ는 ["]{dir="rtl"}등분산 vs 이분산"에서 결정적 증거를 못 줌.
...
![붙여넣은 동영상.png](media/image7.png){width="4.5in"
height="2.737389545056868in"}

+-----------------------------------------------------------------------+
| from statsmodels.stats.diagnostic import het_breuschpagan, het_white  |
|                                                                       |
| \# \-\-- WLS 잔차 (weighted residuals) \-\--                          |
|                                                                       |
| resid_wls = wls_model.resid                                           |
|                                                                       |
| exog_wls = wls_model.model.exog \# 설명변수 행렬                      |
|                                                                       |
| \# \-\-- Breusch--Pagan test \-\--                                    |
|                                                                       |
| bp_lm, bp_lm_p, bp_f, bp_f_p = het_breuschpagan(resid_wls, exog_wls)  |
|                                                                       |
| print(\"\\n\[WLS Breusch--Pagan\]\")                                  |
|                                                                       |
| print(f\"LM stat = {bp_lm:.3f}, p = {bp_lm_p:.4g}\")                  |
|                                                                       |
| print(f\"F stat = {bp_f:.3f}, p = {bp_f_p:.4g}\")                     |
|                                                                       |
| \# \-\-- White test \-\--                                             |
|                                                                       |
| w_lm, w_lm_p, w_f, w_f_p = het_white(resid_wls, exog_wls)             |
|                                                                       |
| print(\"\\n\[WLS White\]\")                                           |
|                                                                       |
| print(f\"LM stat = {w_lm:.3f}, p = {w_lm_p:.4g}\")                    |
|                                                                       |
| print(f\"F stat = {w_f:.3f}, p = {w_f_p:.4g}\")                       |
+-----------------------------------------------------------------------+

\[WLS Breusch--Pagan\]

LM stat = 102.675, p = 6.938e-20

F stat = 21.172, p = 3.565e-22

\[WLS White\]

LM stat = 252.399, p = 5.857e-39

F stat = 18.336, p = 3.614e-56
...
Durbin Watson 퉁계량
![pastedGraphic.pdf](media/image8.png){width="1.8888888888888888in"
height="0.5322287839020122in"}
...
![1\_\_#\$!@%!#\_\_pastedGraphic.pdf](media/image9.png){width="4.4998611111111115in"
height="1.1227734033245844in"}
...
height="1.4730949256342958in"}

오차 가정 만족하지 않은 회귀모형 이유 파악
...
![붙여넣은 동영상.png](media/image11.png){width="4.5in"
height="2.3844870953630797in"}

이상치 &
영향치![pasted-image.pdf](media/image12.png){width="3.4027777777777777in"
height="1.6832786526684165in"}
...
| \# \-\-- 최종 모형 적합 \-\--                                                     |
|                                                                                   |
| selected = \[\'CHAS\', \'NOX\', \'RM\', \'PTRATIO\', \'B\', \'LSTAT\'\] \# 선택된 |
| 변수 리스트                                                                       |
|                                                                                   |
...
height="3.1956528871391074in"}

![붙여넣은 동영상.png](media/image14.png){width="4.5in"
height="3.373206474190726in"}
...
| \# 이상치 또는 영향치 플래그                                          |
|                                                                       |
| mask_bad = (np.abs(std_resid) \> thr_resid) \| (cooks_d \> thr_cook)  |
| \| (lev \> thr_lev)                                                   |
|                                                                       |
...
추정하였다.![붙여넣은 동영상.png](media/image15.png){width="4.5in"
height="2.7942486876640418in"}
...
|                                                                       |
| pred_int.summary_frame(alpha=0.05)                                    |
+-----------------------------------------------------------------------+
