...
**다차원척도법(Multidimensional Scaling, MDS)**은 n개의 개체 간 **유사성(similarity) 또는 거리(distance) 정보**를 이용하여, 이들을 저차원 공간(주로 2차원)에 배치하는 기법이다.

- **핵심 목적**: 고차원 유사성 구조를 시각적으로 해석 가능한 형태로 축소하되, 원래 자료의 **상대적 거리 구조**를 최대한 보존
- **원리**: 유사할수록 가깝게, 유사하지 않을수록 멀리 배치
- **최적화**: Stress 함수(손실 함수)를 최소화하는 좌표를 탐색
:::
...
| 구분 | MDS (다차원척도법) | 군집분석 |
|------|------------------|---------|
| **목적** | 거리 구조를 저차원 공간에 **시각화** | 유사 개체끼리 **집단 분류** |
| **결과물** | 2D/3D 좌표 (지도) | 군집 레이블 |
| **해석 초점** | 개체 간 상대적 거리·패턴 | 집단 내 동질성, 집단 간 이질성 |
| **주요 활용** | 인식 지도(perceptual map), 브랜드 포지셔닝 | 고객 세분화, 질병 분류 |
:::

따라서 군집분석이 집단화의 결과를 제공하는 분류
...
| 구분 | Metric MDS | Non-Metric MDS |
|------|-----------|----------------|
| **입력 척도** | 등간척도·비율척도 (수량적 의미 있는 거리) | 순서척도 (순위 정보만 사용) |
| **거리 변환** | 없음 (거리값 그대로 사용) | 비모수적 단조 변환(isotonic regression) |
| **적합도 평가** | RSQ (R-square) | Stress 값 |
| **알고리즘** | 고유값분해 (한 번에 해결) | SMACOF (반복 수렴) |
| **적합 자료** | 측정형 변수 기반 거리, 등간척도 평가 | 리커트 척도, 순위 자료, 교차빈도표 |
:::
...
것이 타당하다. 예를 들어 "A-B가 A-C보다 더 유사하다"라는
...
단순히 "더 많이 함께 분류되었다"는 서열적 정보를 나타낼 뿐,
...
::: {.callout-note icon=false}
## MDS 기본 알고리즘 (Metric MDS 5단계)

| 단계 | 절차 | 내용 |
|:---:|------|------|
| 1 | **유사성 행렬 계산** | 거리 행렬 $D = [d_{ij}]$ 구성 |
| 2 | **제곱 거리 행렬** | $D^{(2)} = [d_{ij}^2]$ 계산 |
| 3 | **이중 중심화** | $B = -\frac{1}{2}HD^{(2)}H$, $H = I - \frac{1}{n}\mathbf{11}'$ |
| 4 | **고유값분해** | $B = Q\Lambda Q'$로 분해 |
| 5 | **좌표 계산** | $X = Q_r\Lambda_r^{1/2}$ (상위 r개 고유값 사용) |
:::
...
::: {.callout-note icon=false}
## Kruskal Stress 해석 기준

| Stress 값 | 해석 | 적합도 |
|:---:|------|:---:|
| ≤ 0.05 | 매우 좋음 (Excellent fit) | ⭐⭐⭐⭐⭐ |
...
| 변수 | 설명 |
|------|------|
| **Fertility** | 표준화된 출생력 지표 (Ig) |
| **Agriculture** | 남성 중 농업 종사 비율 (%) |
| **Examination** | 군 징집 검사 최고 등급 비율 (%) |
| **Education** | 초등교육 이상 이수 비율 (%) |
| **Catholic** | 가톨릭 인구 비율 (%) |
| **Infant_Mortality** | 영아 사망률 — 1년 미만 생존 비율 (%) |
:::
...
## Stress 값이 높을 때 대처 방법

Stress 값이 높다는 것은 **2차원으로 거리 구조를 충분히 재현하지 못함**을 의미한다.

- 사회경제적 지표처럼 변수 간 상관이 낮으면 2D 재현이 어려움
- **대안**: 3D MDS 적용, Non-Metric MDS 사용, 또는 PCA로 전환
- Stress ≥ 0.20이면 해석에 주의 필요
:::
...
## PCA vs MDS: 계량형 변수에서의 선택

| 기준 | MDS | PCA |
|------|-----|-----|
| **차원 해석** | 거리 보존용 좌표 (해석 불명확) | 부하량으로 차원 의미 명확 |
| **적용 가능 자료** | 거리/유사성 행렬 | 계량형 변수 행렬 |
| **회전 불변성** | 보장 안 됨 | 보장됨 |
| **변수 기여도** | 알 수 없음 | 부하량(loadings)으로 확인 |

👉 **계량형 변수 → PCA 권장** (차원 해석 가능). 순위·유사성 자료 → MDS 권장.
:::
...
**PC1: "전통적·농업 중심성 요인(Traditional--Agricultural
Axis)"**
...
**PC2: "종교적 구성 및 건강지표 요인(Religiosity--Health
Axis)"**
...
::: {.callout-tip icon=false}
## MDS vs 대응분석(CA): 교차빈도표 분석 비교

| 구분 | MDS | 대응분석(CA) |
|------|-----|------------|
| **거리 척도** | 유클리드 거리 등 사용자 정의 | 카이제곱(χ²) 거리 — 범주형에 특화 |
| **행·열 동시 표현** | 가능 (스케일 조정 필요) | 자연스럽게 Biplot으로 표현 |
| **설명력 지표** | Stress | 총 관성 대비 설명 비율 |
| **특화 분야** | 일반적 거리 구조 시각화 | **범주형 교차표 분석에 최적** |
:::
...
::: {.callout-note icon=false}
## 대응분석(Correspondence Analysis, CA) 정의

**대응분석**은 **교차표(분할표)** 형태의 범주형 자료에서, 행·열 범주를 동시에 저차원 공간(주로 2차원)에 좌표로 나타내어 **범주 간 구조적 관계와 연관성**을 탐색하는 기법이다.

| 특성 | 내용 |
|------|------|
| **입력 자료** | 교차표 (빈도 행렬) |
| **출력** | 행·열 범주의 2D 좌표 (Biplot) |
| **거리 척도** | 카이제곱(χ²) 거리 |
| **해석 원리** | 가까운 범주 = 연관성 강함, 원점 = 전국 평균 패턴 |
| **분해 방법** | 특이값 분해(SVD) 기반 |
:::
...
귀무가설 : $H_{0}:\pi_{ij} = \pi_{kj}\ \text{for all}\ j = 1,2,\ldots,C$
...
$$TS = \sum_{i,j}\frac{(O_{ij} - E_{ij})^{2}}{E_{ij}} \sim \chi^{2}\big((R-1)(C-1)\big)$$

- $O_{ij} = n_{ij}$: 관측빈도, $E_{ij} = \frac{n_{i+}n_{+j}}{n_{++}}$: 기대빈도
- 동질성 검정과 독립성 검정 모두 **동일한 검정통계량** 사용

**Cochran 정리** — 적용 조건: 기대빈도 5 이하인 셀이 전체의 20% 미만일 것.
위반 시 **Fisher Exact 검정** 적용.
:::
...
$(i,j)$셀 빈도 $n_{ij}$의 행별 합 $n_{i+}$, 열별 합 $n_{+j}$, 전체 $n_{++}$

| 단계 | 절차 | 내용 |
|:---:|------|------|
| 1 | **행 프로파일 계산** | $f_{ij} = \dfrac{n_{ij}}{n_{i+}}$ — 행별 상대빈도 |
| 2 | **행 좌표 (가중 유클리드 공간)** | $R_i = \left(\dfrac{f_{i1}}{f_{i+}},\ldots,\dfrac{f_{iC}}{f_{i+}}ight)$ |
| 3 | **카이제곱 거리** | $D(R_i,R_j)=\sqrt{\sum_{c}rac{\left(rac{f_{ic}}{f_{i+}}-rac{f_{jc}}{f_{j+}}ight)^2}{f_{+c}}}$ |
| 4 | **열 프로파일 동일 처리** | 열 프로파일도 동일 방식으로 좌표 계산 → 행·열 동시 2D 표현 |
:::
### 사례분석
...
| 원칙 | 해석 |
|------|------|
| **가까운 행·열 범주** | 해당 행 범주와 열 범주 간 **강한 연관성** |
| **원점에 가까운 범주** | 전체 평균 패턴에 가까운 범주 (특별한 연관 없음) |
| **원점에서 멀리 떨어진 범주** | 특정 방향으로 편중된 강한 특성 |
| **같은 방향의 범주** | 유사한 패턴을 공유 |
| **제1차원** | 가장 많은 관성(정보)을 설명하는 주 구분 축 |
:::
