...
**판별분석**은 사전에 집단 정보가 **알려진 상태**에서, 개체의 여러 특성값(변수)을 이용하여 **집단 간 차이를 설명하고 새로운 개체가 어느 집단에 속할지를 예측**하는 지도학습(supervised learning) 기법이다.

| 구분 | 내용 |
|------|------|
| **목적** | 판별함수 추정 → 새로운 개체 분류 |
| **전제 조건** | 집단 레이블 사전 제공 |
| **대표 기법** | Fisher LDA, QDA, 로지스틱 판별분석 |
| **이론적 기반** | Bayes 의사결정 규칙 (ECM 최소화) |
:::
...
| **대표 방법** | Fisher LDA, 로지스틱 판별 | K-평균, 계층적 군집 |
:::
...
![](images/da_concept.png){fig-align="center" width="40%"}
...
![](images/da_ldaqda){fig-align="center" width="80%"}
...
|  | 예측: 안전(Positive) | 예측: 불안전(Negative) |
|------|:---:|:---:|
| **실제: 안전(Safe)** | TP (정분류) | FN (2종 오류) |
| **실제: 불안전(Unsafe)** | FP (1종 오류) | TN (정분류) |
:::
...
| 지표 | 수식 | 해석 |
|------|------|------|
| **정확도 (Accuracy)** | $(TP+TN)/(TP+TN+FP+FN)$ | 전체 중 정확히 분류된 비율 |
| **민감도 (Sensitivity/Recall)** | $TP/(TP+FN)$ | 실제 양성 중 양성으로 올바르게 판별한 비율 |
| **특이도 (Specificity)** | $TN/(TN+FP)$ | 실제 음성 중 음성으로 올바르게 판별한 비율 |
| **양성예측도 (PPV)** | $TP/(TP+FP)$ | 양성 예측 중 실제 양성 비율 |
| **음성예측도 (NPV)** | $TN/(TN+FN)$ | 음성 예측 중 실제 음성 비율 |
| **오분류율 (Error Rate)** | $(FP+FN)/(TP+TN+FP+FN)$ | 잘못 분류된 비율 |
| **F1 점수** | $2TP/(2TP+FP+FN)$ | Precision과 Recall의 조화평균 — 불균형 데이터에 유용 |
| **Balanced Accuracy** | $(Sensitivity+Specificity)/2$ | 불균형 클래스일 때 정확도 보정 지표 |
| **AUC** | ROC 곡선 하 면적 | 다양한 임계값에서 모델의 전반적 판별 능력 |
:::


#### ROC 곡선 (Receiver Operating Curve)

아래 그림은 진단 모형의 성능을 평가하기 위해 x축을 (1 − 특이도), y축을 민감도로 하여 나타낸 이차원 곡선(ROC Curve) 이다. 이 그래프는
양성과 음성을 가장 정확하게 구분하기 위한 기준값(cut-off value) 을
결정하는 데 활용되며, 의학 및 생명과학 분야에서 진단검사의 정확도를
비교하는 표준적인 도구로 사용된다.

![](images/da_roc.png){fig-align="center" width="60%"}
...
원점 (0,0)은 민감도 0, 특이도 1, 즉 "모두 음성으로 판단하는
완전한 보수적 진단"을 의미한다. 반대로 (1,0)은 민감도 1, 특이도 0으로,
"모두 양성으로 판단하는 진단"을 뜻한다. (1,1)은 민감도와
...
| AUC 값 | 판별 능력 |
|:---:|------|
| 0.90 ~ 1.00 | 매우 우수 (Excellent) |
| 0.80 ~ 0.90 | 우수 (Good) |
| 0.70 ~ 0.80 | 양호 (Fair) |
| 0.60 ~ 0.70 | 미흡 (Poor) |
| 0.50 ~ 0.60 | 실패 (Fail) |
| = 0.50 | 무작위 (Random) — 판별력 없음 |
:::

따라서, AUC는 0.5보다 커야 의미 있는 진단방법이며, AUC 값이 가장 큰
모형이 가장 적절한 진단방법으로 간주된다.

**기준값(Cut-off value)의 결정**

ROC 곡선에서 양성과 음성을 구분하는 기준값(c) 을 선택하는 가장 일반적인
방법은 Youden의 J 통계량(Youden's Index) 을 활용하는
것이다.
...
| 방법 | 절차 | 편향 | 권장 상황 |
|------|------|:---:|---------|
| **Re-substitution** | 학습 데이터로 판별 후 동일 데이터 적용 | 낙관 편향 (과대추정) | 소프트웨어 기본값; 큰 데이터만 |
| **테스트 데이터** | 데이터를 1/2 분할 → 한쪽으로 학습, 나머지로 검증 | 비효율 (절반만 사용) | **빅데이터에서 가장 많이 사용** |
| **Cross-validation (Jackknife)** | 개체 하나씩 제외하며 판별식 추정 | 거의 없음 | **소표본에서 권장; 가장 정확** |
:::
...
사용한다. 빅데이터에서는 train(학습), test(검증) 데이터로 나누어
판별규칙 검증한다.
...
당시 타이타닉은 "절대 침몰하지 않는 배(The unsinkable
...
![](images/da_penguin.png){fig-align="center" width="80%"}
...
::: {.callout-note icon=false}
## Palmer Penguins 데이터 변수 설명

| 변수명 | 유형 | 단위 | 설명 |
|--------|:---:|:---:|------|
| **species** | 범주형 | — | 펭귄의 종 (Adélie, Chinstrap, Gentoo) |
| **island** | 범주형 | — | 관찰된 섬 (Biscoe, Dream, Torgersen) |
| **bill_length_mm** | 연속형 | mm | 부리 길이. 종 간 형태적 차이 분석에 활용 |
| **bill_depth_mm** | 연속형 | mm | 부리 깊이. 먹이 습성 및 종 분류에 영향 |
| **flipper_length_mm** | 연속형 | mm | 지느러미 길이. 종별 크기 및 생태적 차이 |
| **body_mass_g** | 연속형 | g | 몸무게. 생리적 상태 및 종 차이 파악 |
| **sex** | 범주형 | — | 성별 (male / female) |
| **year** | 정수형 | 년도 | 측정 연도 (2007, 2008, 2009) |
:::
...
분류비용 Cost of Misclassification: $c(j \mid i)$, 모집단 i에 속한 개체를
...
![](images/da_classify.png){fig-align="center" width="50%"}
...
오분류로 인한 손실까지 고려한 "경제적 판별기준"을 제공한다.
...
![](images/da_ldaconcept.png){fig-align="center" width="80%"}
...
![](images/da_qdaconcept.png){fig-align="center" width="80%"}
...
![](images/da_titanic_scatter.png){fig-align="center" width="80%"}
...
::: {.callout-tip icon=false}
## LDA vs QDA 비교 (Titanic 데이터 결과)

| 구분 | LDA | QDA |
|------|:---:|:---:|
| **공분산 가정** | 동일 ($\Sigma_1=\Sigma_2$) | 서로 다름 ($\Sigma_1 \neq \Sigma_2$) |
| **Box's M 결과** | 등분산 가정 위배 | 적용 가능 |
| **전체 정확도** | 65.60% | 67.00% |
| **생존자 재현율** | 26% | 29% |
| **경계 형태** | 선형 | 이차(비선형) |

👉 Box's M이 유의하면 → **QDA 선택** 권장
:::
...
![](images/da_titanic_pca.png){fig-align="center" width="100%"}
...
방향으로 나타난다. 가족 규모가 클수록 PC1 점수는 증가한다. 요금이 높은 승객도 상대적으로 PC1이 증가하는 경향을 보인다. 반대로 나이가 많을수록 PC1 점수는 감소한다.

따라서 PC1은 "가족 규모 및 경제적 여건을 반영하는
축(Family-size / Fare axis)" 또는 "젊고 가족이 많은 승객 vs 나이가 많고 가족 동반이 적은 승객"을 구분하는 축으로 해석할 수 있다.
...
가로축은 False Positive Rate(FPR)=('정답을 얼마나 잘
맞추는가')=민감도, 세로축은 True Positive
Rate(TPR)=('실패를 성공으로 잘못 분류한
비율')=(1-특이도)를 나타내며, 기준값을 0에서 1까지
...
![](images/da_titanic_roc.png){fig-align="center" width="60%"}
...
| 값 | 해석 |
|:---:|------|
| J = 1 | 완벽한 분류기 (민감도=1, 특이도=1) |
| J = 0 | 무작위 분류기와 동일 (분별력 없음) |
| J > 0 | 실제 판별 능력 있음 |

**최적 기준값**: $c^* = \arg\max_c J(c)$ — Youden Index를 최대화하는 cutoff 선택
:::
...
적절하지 않음을 수치적으로 판단할 수 있다. 간단한 계산으로 모델의 가장 균형 잡힌 cutoff 제시 → 0.5라는 임의 기준에 의존하지 않는다.
...
되는 cutoff를 선택하여 "성공/실패",
"질병/정상", "위험군/비위험군" 등을 보다
합리적으로 분류할 수 있다.
...
::: {.callout-tip icon=false}
## Fisher 판별분석 vs 로지스틱 판별분석 비교

| 구분 | Fisher LDA | 로지스틱 판별분석 |
|------|-----------|----------------|
| **분포 가정** | 다변량 정규분포 + 등분산 | 없음 (분포 자유) |
| **출력값** | 판별점수 (확률 아님) | 사후확률 (0~1) |
| **ROC/cutoff 분석** | 간접적 | 자연스럽게 가능 |
| **범주형 변수** | 제한적 | 자유롭게 포함 가능 |
| **계수 해석** | 판별 방향 | 오즈비(OR)로 해석 |
| **확장성** | 제한적 | GLM 계열 — 다항·순서형 등 확장 용이 |
:::

다음에서는 Fisher 방법에 비해 로지스틱
...
![](images/da_titanic_youden.png){fig-align="center" width="60%"}
...
![](images/da_titanic_logit.png){fig-align="center" width="100%"}
...
동반한 가족 수가 많을수록 생존에 불리한 영향을 준다. OR = 0.7442, 즉 sibsp가 1명 늘어날 때 생존할 오즈는 약 26% 감소. → 많은
...
OR = 11.6851 (95% CI: 8.4833 \~ 16.0954) 여성의 생존할 odds는 남성 대비 약 11.7배 높다. 이는 Titanic 생존 자료에서 가장 잘 알려진 특징인
"여성 우선(women and children first)" 규칙을 반영한 강력한 효과이다. female은 생존 여부를 결정하는 가장 강력한 단일 설명변수이다.

훈련 정확도: 0.7741626794258373

![](images/da_titanic_logit02.png){fig-align="center" width="100%"}
...
::: {.callout-note icon=false}
## 다집단 판별 규칙 체계: Bayes → MAP → ECM → LDA/QDA

| 규칙 | 수식 | 특징 |
|------|------|------|
| **Bayes** | $\hat{g}(x) = \arg\max_g \pi_g f_g(x)$ | 오분류 확률 최소화 |
| **MAP** | $\hat{g}(x) = \arg\max_g P(G=g \mid x)$ | 사후확률 최대화 (Bayes와 동치) |
| **ECM** | $\hat{g}(x) = \arg\max_g \delta_g(x) = \arg\max_g \ln(\pi_g f_g(x))$ | 판별함수 형태로 계산 가능화 |
| **LDA** | 정규분포 + 공통 공분산 적용 → **선형** 판별경계 | 실용적, 계산 안정 |
| **QDA** | 정규분포 + 집단별 공분산 적용 → **이차** 판별경계 | 유연, 표본 충분 시 우월 |

세 규칙은 동일한 선택을 하며, LDA·QDA는 ECM에 정규모형을 대입한 특수 형태이다.
:::
...
"절편이 하나가 아니라 (집단 수 -- 1)로 모집단 개수보다 1개
적음"은 다음과 같이 표현할 수 있다.

"다항 로지스틱 회귀에서는 기준집단을 제외한 K-1개의
...
Box's M 검정에서 유의확률 p-value가 0.000으로 나타나, 세
...
=== Multinomial Logistic Regression Estimated Coefficients ===
<br>
[참조 대비: Adelie 로짓함수]
<br>
...
날개길이와 체중은 계수 크기가 매우 작고 오즈비는 1.01 수준으로, Adelie 구분에서는 상대적으로 영향력이 낮다.
정리: Adelie는 ‘짧고 깊은 부리’라는 조합이 결정적으로 구분되는 특징이며, 체중·날개길이는 판별력이 상대적으로 작다.
...
**Gentoo는** 신체 크기가 큰 종이므로, 결과도 이를 반영한다.
부리길이는 계수 -0.106(오즈비 0.90)**으로 작은 음(-) 효과를 보이지만 큰 판별력은 아니다.
